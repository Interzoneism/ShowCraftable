...
public class BlockEntityBaseReturnTeleporter : BlockEntity
{
	...
	public override void Initialize(ICoreAPI api)
	{
		...
		if (api.World.Side == EnumAppSide.Client)
		{
			...
			animUtil.InitializeAnimator("basereturnteleporter", null, null, new Vec3f(0f, rotateY, 0f));
	...
	private void OnServerGameTick(float dt)
	{
		...
		{
			activated = false;
			IServerPlayer serverPlayer = Api.World.NearestPlayer((double)Pos.X + 0.5, (double)Pos.InternalY + 0.5, (double)Pos.Z + 0.5) as IServerPlayer;
			if (serverPlayer.Entity.Pos.DistanceTo(Pos.ToVec3d().Add(0.5, 0.0, 0.5)) < 5.0)
			{
				FuzzyEntityPos spawnPosition = serverPlayer.GetSpawnPosition(consumeSpawnUse: false);
				serverPlayer.Entity.TeleportToDouble(spawnPosition.X, spawnPosition.Y, spawnPosition.Z);
				Api.World.PlaySoundAt(new AssetLocation("sounds/effect/translocate-breakdimension"), serverPlayer.Entity.Pos.X, serverPlayer.Entity.Pos.InternalY, serverPlayer.Entity.Pos.Z, null, randomizePitch: false, 16f);
			}
	...
	protected void HandleSoundClient(float dt)
	{
		...
		if (activated)
		{
			...
			if (coreClientAPI != null && coreClientAPI.World.Player.Entity.Pos.DistanceTo(Pos.ToVec3d().Add(0.5, 0.0, 0.5)) < 5.0)
			{
