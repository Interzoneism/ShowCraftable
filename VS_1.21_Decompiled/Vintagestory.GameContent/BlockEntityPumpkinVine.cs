...
public class BlockEntityPumpkinVine : BlockEntity
{
	...
	private void TryGrowPumpkins()
	{
		...
		{
			double num = pumpkinTotalHoursForNextStage[blockFacing];
			while (num > 0.0 && Api.World.Calendar.TotalHours > num)
			{
				...
				if (IsPumpkin(block))
				{
					int num2 = CurrentPumpkinStage(block);
					if (num2 == 4)
					{
						num = 0.0;
					}
					...
					{
						SetPumpkinStage(block, blockPos, num2 + 1);
						num += (double)pumpkinHoursToGrow;
					}
				}
				else
				{
				...
				pumpkinTotalHoursForNextStage[blockFacing] = num;
			}
		}
	}
	...
	private void TrySpawnPumpkin(double curTotalHours)
	{
		...
		foreach (BlockFacing blockFacing in hORIZONTALS)
		{
			BlockPos blockPos = Pos.AddCopy(blockFacing);
			Block block = Api.World.BlockAccessor.GetBlock(blockPos);
			if (CanReplace(block) && PumpkinCropBehavior.CanSupportPumpkin(Api, blockPos.DownCopy()))
			{
				Api.World.BlockAccessor.SetBlock(pumpkinBlock.BlockId, blockPos);
				pumpkinTotalHoursForNextStage[blockFacing] = curTotalHours + (double)pumpkinHoursToGrow;
				break;
			}
		}
	}
	...
	private bool IsValidParentBlock(Block parentBlock)
	{
		if (parentBlock != null)
		{
			...
			if (name.StartsWithOrdinal("crop-pumpkin") || name.StartsWithOrdinal("pumpkin-vine"))
			{
				return true;
			}
		}
	...
	{
		Block block = Api.World.GetBlock(pumpkinBlock.CodeWithParts(toStage.ToString() ?? ""));
		if (block != null)
		{
	...
	private void TrySpawnNewVine()
	{
		...
		if (CanGrowOn(Api, blockPos))
		{
			...
			if (Api.World.BlockAccessor.GetBlockEntity(blockPos) is BlockEntityPumpkinVine blockEntityPumpkinVine)
			{
				blockEntityPumpkinVine.CreatedFromParent(Pos, vineSpawnDirection, totalHoursForNextStage);
			}
		}
	}
	...
	public override void FromTreeAttributes(ITreeAttribute tree, IWorldAccessor worldForResolving)
	{
		...
		foreach (BlockFacing blockFacing in hORIZONTALS)
		{
			pumpkinTotalHoursForNextStage[blockFacing] = tree.GetDouble(blockFacing.Code);
		}
	...
	public override void ToTreeAttributes(ITreeAttribute tree)
	{
		...
		foreach (BlockFacing blockFacing in hORIZONTALS)
		{
			tree.SetDouble(blockFacing.Code, pumpkinTotalHoursForNextStage[blockFacing]);
		}
