...
public class BlockFarmland : Block
{
	...
	public override void OnLoaded(ICoreAPI api)
	{
		...
		if (Attributes != null)
		{
			...
			if (WeedNames == null)
			{
				...
				int num = 0;
				while (WeedNames != null && num < WeedNames.Length)
				{
					TotalWeedChance += WeedNames[num].Chance;
					num++;
				}
			}
		}
	...
	{
		if (world.BlockAccessor.GetBlockEntity(blockSel.Position) is BlockEntityFarmland blockEntityFarmland && blockEntityFarmland.OnBlockInteract(byPlayer))
		{
			return true;
		}
	...
	{
		if (world.BlockAccessor.GetBlockEntity(pos) is BlockEntityFarmland blockEntityFarmland)
		{
			return new ItemStack(api.World.GetBlock(CodeWithVariant("state", blockEntityFarmland.IsVisiblyMoist ? "moist" : "dry"))).GetName();
		}
