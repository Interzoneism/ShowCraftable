...
public class FruitRendererSystem : IRenderer, IDisposable
{
	...
	public void AddFruit(Item fruit, Vec3d position, FruitData data)
	{
		if (fruit.Shape != null)
		{
			if (!renderers.TryGetValue(fruit.Code, out var value))
			{
				value = new FruitRenderer(capi, fruit);
				renderers.Add(fruit.Code, value);
			}
			value.AddFruit(position, data);
		}
	}

	public void RemoveFruit(Item fruit, Vec3d position)
	{
		renderers.TryGetValue(fruit.Code, out var value);
		value?.RemoveFruit(position);
	}
