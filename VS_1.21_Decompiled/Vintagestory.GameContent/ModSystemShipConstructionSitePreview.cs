...
public class ModSystemShipConstructionSitePreview : ModSystem
{
	...
	private void onTick(float dt)
	{
		if (capi.World.Player.InventoryManager.ActiveHotbarSlot.Itemstack?.Collectible is ItemRoller)
		{
			...
			List<BlockPos> blocks = ItemRoller.siteListByFacing[orient];
			List<BlockPos> blocks2 = ItemRoller.waterEdgeByFacing[orient];
			int item = ColorUtil.ColorFromRgba(0, 50, 150, 50);
			capi.World.HighlightBlocks(capi.World.Player, 941, blocks, EnumHighlightBlocksMode.AttachedToSelectedBlock, EnumHighlightShape.Cube);
			capi.World.HighlightBlocks(capi.World.Player, 942, blocks2, new List<int> { item }, EnumHighlightBlocksMode.AttachedToSelectedBlock, EnumHighlightShape.Cube);
		}
		else
		{
