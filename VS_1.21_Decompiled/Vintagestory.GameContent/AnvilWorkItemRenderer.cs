...
public class AnvilWorkItemRenderer : IRenderer, IDisposable
{
	...
	private void RenderRecipeOutLine()
	{
		if (recipeOutlineMeshRef != null && !api.HideGuis)
		{
			...
			float num = (render.LineWidth = 2f * api.Settings.Float["wireframethickness"]);
	...
	public void RegenMesh(ItemStack workitemStack, byte[,,] voxels, bool[,,] recipeToOutlineVoxels)
	{
		...
		if (workitemStack != null)
		{
			...
			MeshData data = ItemWorkItem.GenMesh(api, workitemStack, voxels, out texId);
		...
		for (int j = 0; j < 16; j++)
		{
			for (int k = 0; k < 6; k++)
			{
				for (int l = 0; l < 16; l++)
				{
					...
					if ((!flag || enumVoxelMaterial != EnumVoxelMaterial.Metal) && (flag || enumVoxelMaterial != EnumVoxelMaterial.Empty))
					{
