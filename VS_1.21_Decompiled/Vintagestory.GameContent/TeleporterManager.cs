...
public class TeleporterManager : ModSystem
{
	...
	{
		if (Locations.TryGetValue(pos, out var value))
		{
			return value;
		}
		value = new TeleporterLocation
		{
			SourceName = "Location-" + (Locations.Count + 1),
			SourcePos = pos.Copy()
		};
		Locations[value.SourcePos] = value;
		return value;
	}
	...
	private TextCommandResult handleSetTlPos(TextCommandCallingArgs args)
	{
		...
		{
			sapi.World.BlockAccessor.SetBlock(sapi.World.GetBlock(block.CodeWithVariant("state", "normal")).Id, asBlockPos);
		}
		if (!(sapi.World.BlockAccessor.GetBlockEntity(asBlockPos) is BlockEntityStaticTranslocator blockEntityStaticTranslocator))
		{
		...
		bool fullyRepaired = blockEntityStaticTranslocator.FullyRepaired;
		if (!fullyRepaired)
		{
			blockEntityStaticTranslocator.findNextChunk = false;
			blockEntityStaticTranslocator.repairState = blockEntityStaticTranslocator.RepairInteractionsRequired;
		}
		blockEntityStaticTranslocator.tpLocation = (args[1] as Vec3d).AsBlockPos;
		blockEntityStaticTranslocator.tpLocationIsOffset = !args.Parsers[2].IsMissing && (bool)args[2];
		if (!fullyRepaired)
		{
			blockEntityStaticTranslocator.setupGameTickers();
		}
		blockEntityStaticTranslocator.DoActivate();
		return TextCommandResult.Success((blockEntityStaticTranslocator.tpLocationIsOffset ? "Relative " : "") + "Target position set.");
	}
	...
	private void OnSetLocationReceived(IServerPlayer fromPlayer, TeleporterLocation networkMessage)
	{
		...
		if (sapi.World.BlockAccessor.GetBlockEntity(networkMessage.SourcePos) is BlockEntityTeleporter blockEntityTeleporter)
		{
			blockEntityTeleporter.MarkDirty();
		}
	}
	...
	private void OnSetValuesDialog(string elementCode, string newValue)
	{
		...
		case "targetlocation":
			if (newValue.Length > 0)
			{
				...
				forLocation.TargetPos = targetPositionsOrdered[index];
				forLocation.TargetName = targetNamesOrdered[index];
			}
			break;
		case "cancel":
			dialog.TryClose();
	...
	private string OnGetValuesDialog(string elementCode)
	{
		...
		case "targetlocation":
		{
			...
			int num = 0;
			foreach (BlockPos item in targetPositionsOrdered)
			{
				list.Add(num++);
				targetNamesOrdered.Add(Locations[item].SourceName);
			}
	...
	private void OnConfigEventServer(string eventName, ref EnumHandling handling, IAttribute data)
	{
		...
		TeleporterLocation orCreateLocation = GetOrCreateLocation(new BlockPos(treeAttribute.GetInt("posX"), treeAttribute.GetInt("posY"), treeAttribute.GetInt("posZ")));
		IServerPlayer serverPlayer = sapi.World.PlayerByUid(treeAttribute.GetString("playerUid")) as IServerPlayer;
		serverChannel.SendPacket(new TpLocations
		{
			ForLocation = orCreateLocation,
			Locations = Locations
		}, serverPlayer);
	}
