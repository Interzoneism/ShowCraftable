...
public class BECheese : BlockEntityContainer
{
	...
	public ItemStack TakeSlice()
	{
		...
		ItemCheese itemCheese = inv[0].Itemstack.Collectible as ItemCheese;
		MarkDirty(redrawOnClient: true);
		switch (itemCheese.Part)
		{
		case "1slice":
		{
		...
		case "2slice":
		{
			ItemStack itemStack3 = new ItemStack(Api.World.GetItem(itemCheese.CodeWithVariant("part", "1slice")));
			inv[0].Itemstack = itemStack3;
			return itemStack3.Clone();
		}
		case "3slice":
		{
			ItemStack itemStack2 = new ItemStack(Api.World.GetItem(itemCheese.CodeWithVariant("part", "1slice")));
			inv[0].Itemstack = new ItemStack(Api.World.GetItem(itemCheese.CodeWithVariant("part", "2slice")));
			return itemStack2.Clone();
		}
		case "4slice":
		{
			ItemStack itemStack = new ItemStack(Api.World.GetItem(itemCheese.CodeWithVariant("part", "1slice")));
			inv[0].Itemstack = new ItemStack(Api.World.GetItem(itemCheese.CodeWithVariant("part", "3slice")));
	...
	public override bool OnTesselation(ITerrainMeshPool mesher, ITesselatorAPI tessThreadTesselator)
	{
		...
		modeldata.Scale(new Vec3f(0.5f, 0f, 0.5f), 1f, 1f, 1f);
