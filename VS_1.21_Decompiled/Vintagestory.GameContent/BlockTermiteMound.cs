...
public class BlockTermiteMound : BlockRequireSolidGround
{
	...
	public override void OnLoaded(ICoreAPI api)
	{
		...
		Block block = api.World.GetBlock(new AssetLocation("rock-" + Variant["rock"]));
		(islarge ? largeTermiteBlockCodeByRockid : mediumTermiteBlockCodeByRockid)[block.Id] = api.World.GetBlock(CodeWithVariant("rock", Variant["rock"]));
		base.OnLoaded(api);
	}
	...
	public override bool TryPlaceBlockForWorldGen(IBlockAccessor blockAccessor, BlockPos pos, BlockFacing onBlockFace, IRandom worldGenRand, BlockPatchAttributes attributes = null)
	{
		...
		int num = 32;
		int key = blockAccessor.GetMapChunkAtBlockPos(pos).TopRockIdMap[pos.Z % num * num + pos.X % num];
		Block value = null;
		if (islarge)
		{
			largeTermiteBlockCodeByRockid.TryGetValue(key, out value);
		}
		else
		{
			mediumTermiteBlockCodeByRockid.TryGetValue(key, out value);
		}
		if (value != null)
		{
			blockAccessor.SetBlock(value.Id, pos);
		}
		return true;
	}
}
