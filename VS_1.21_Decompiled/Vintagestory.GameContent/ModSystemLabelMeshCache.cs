...
public class ModSystemLabelMeshCache : ModSystem
{
	...
	public void RequestLabelTexture(int labelColor, BlockPos pos, ItemStack labelStack, Action<int> onLabelTextureReady)
	{
		...
		{
			value.UsedCounter.Add(pos.GetHashCode());
			if (value.TextureSubId != 0)
			{
				onLabelTextureReady(value.TextureSubId);
			}
			else
			{
				value.onLabelTextureReady.Add(onLabelTextureReady);
			}
			return;
		}
		...
		{
			if (itemStackRenders.TryGetValue(hashCode, out var value2))
			{
				value2.TextureSubId = texSubid;
				foreach (Action<int> item in value2.onLabelTextureReady)
				{
					item(texSubid);
				}
				value2.onLabelTextureReady.Clear();
	...
	public void FreeLabelTexture(ItemStack labelStack, int labelColor, BlockPos pos)
	{
		...
		int key = labelStack.GetHashCode(GlobalConstants.IgnoredStackAttributes) + 23 * labelColor.GetHashCode();
		if (itemStackRenders.TryGetValue(key, out var value))
		{
			value.UsedCounter.Remove(pos.GetHashCode());
			if (value.UsedCounter.Count == 0)
			{
				capi.BlockTextureAtlas.FreeTextureSpace(value.TextureSubId);
				value.TextureSubId = 0;
				itemStackRenders.Remove(key);
			}
		}
	}
}
