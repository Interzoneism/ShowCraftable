...
public class SurvivalCoreSystem : ModSystem
{
	...
	private SurvivalConfig config = new SurvivalConfig();

	public IShaderProgram anvilShaderProg;

	public Dictionary<string, MetalPropertyVariant> metalsByCode;

	public float EarthAxialTilt = 0.40910518f;

	private HashSet<string> createdPlayers = new HashSet<string>();
	...
	{
		api.Assets.AddModOrigin("game", Path.Combine(GamePaths.AssetsPath, "survival"), (api.Side == EnumAppSide.Client) ? "textures" : null);
	}

	public override void Start(ICoreAPI api)
	{
		GameVersion.EnsureEqualVersionOrKillExecutable(api, FileVersionInfo.GetVersionInfo(Assembly.GetExecutingAssembly().Location).FileVersion, "1.21.0", "VSSurvivalMod");
		this.api = api;
		...
		{
			coreServerAPI.WorldManager.SetBlockLightLevels(config.BlockLightLevels);
			coreServerAPI.WorldManager.SetSunLightLevels(config.SunLightLevels);
			coreServerAPI.WorldManager.SetSunBrightness(config.SunBrightness);
			coreServerAPI.Event.SaveGameLoaded += applySeasonConfig;
		}
		else
		{
	...
	public override void AssetsLoaded(ICoreAPI api)
	{
		...
		foreach (MetalProperty value in api.Assets.GetMany<MetalProperty>(api.Logger, "worldproperties/block/metal.json").Values)
		{
			for (int i = 0; i < value.Variants.Length; i++)
			{
				MetalPropertyVariant metalPropertyVariant = value.Variants[i];
				metalsByCode[metalPropertyVariant.Code.Path] = metalPropertyVariant;
			}
		}
		if (api is ICoreClientAPI coreClientAPI)
		{
			IAsset asset = api.Assets.TryGet("config/colorpresets.json");
			coreClientAPI.ColorPreset.Initialize(asset);
		}
	}
	...
	public override void StartServerSide(ICoreServerAPI api)
	{
		...
		api.Event.ServerRunPhase(EnumServerRunPhase.GameReady, delegate
		{
		...
		EntityBehaviorPassivePhysicsMultiBox.InitServer(api);
	}
	...
	{
		config = networkMessage;
		applyConfig();
	}

	private EnumHemisphere GetHemisphere(double posX, double posZ)
	{
		if (!(api.World.Calendar.OnGetLatitude(posZ) > 0.0))
		{
			return EnumHemisphere.South;
		}
	...
	{
		double num = api.World.Calendar.OnGetLatitude(posZ) * Math.PI / 2.0;
		float value = (float)Math.PI * 2f * (dayRel - 0.5f);
		double num2 = (double)(0f - EarthAxialTilt) * Math.Cos((float)Math.PI * 2f * (yearRel + 0.02739726f));
		double num3 = Math.Sin(num);
		double num4 = Math.Cos(num);
		double num5 = Math.Sin(num2);
		double num6 = GameMath.Clamp(num3 * num5 + num4 * GameMath.Cos(num2) * (double)GameMath.Cos(value), -1.0, 1.0);
		double num7 = Math.Sqrt(1.0 - num6 * num6);
		float num8 = (float)Math.Acos(GameMath.Clamp((num3 * num6 - num5) / (num4 * num7 + 1.0000000116860974E-07), -1.0, 1.0));
		num8 = ((float)Math.PI + (float)Math.Sign(value) * num8) % ((float)Math.PI * 2f);
		return new SolarSphericalCoords((float)Math.PI * 2f - (float)Math.Acos(num6), (float)Math.PI * 2f - num8);
	}
	...
	private void Event_PlayerPlaying(IServerPlayer byPlayer)
	{
		...
		if (byPlayer.WorldData.CurrentGameMode != EnumGameMode.Creative && byPlayer.WorldData.CurrentGameMode != EnumGameMode.Spectator)
		{
	...
	private void applyConfig()
	{
		...
		if (api.Side == EnumAppSide.Server)
		{
			foreach (CollectibleObject collectible in api.World.Collectibles)
			{
				...
				{
					collectible.Durability = (int)((float)collectible.Durability * config.ToolDurabilityModifier);
				}
			}
		}
		applySeasonConfig();
	...
	private void applySeasonConfig()
	{
		if (api.World.Calendar != null)
		{
			string text = api.World.Config.GetString("seasons");
			if (text == "spring")
			{
