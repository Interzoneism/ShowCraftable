...
public class BlockEntityFirepit : BlockEntityOpenableContainer, IHeatSource, IFirePit, ITemperatureSensitive
{
	...
	{
		CombustibleProperties combustibleProperties = fuelCombustibleOpts;
		...
		{
			return (float)combustibleProperties.BurnTemperature * HeatModifier > 0f;
		}
		return false;
	}
	...
	public void heatInput(float dt)
	{
		...
		{
			float num2 = (1f + GameMath.Clamp((furnaceTemperature - inputStackTemp) / 30f, 0f, 1.6f)) * dt;
			if (num >= meltingPoint)
			{
				num2 /= 11f;
			}
			float num3 = changeTemperature(inputStackTemp, furnaceTemperature, num2);
			int num4 = Math.Max((inputStack.Collectible.CombustibleProps != null) ? inputStack.Collectible.CombustibleProps.MaxTemperature : 0, (inputStack.ItemAttributes?["maxTemperature"] != null) ? inputStack.ItemAttributes["maxTemperature"].AsInt() : 0);
			if (num4 > 0)
			{
				num3 = Math.Min(num4, num3);
			}
			if (inputStackTemp != num3)
			{
				InputStackTemp = num3;
				num = num3;
			}
		...
		if (num >= meltingPoint)
		{
			float num5 = num / meltingPoint;
			inputStackCookingTime += (float)GameMath.Clamp((int)num5, 1, 30) * dt;
		}
	...
	public void heatOutput(float dt)
	{
		...
		if (outputStackTemp < furnaceTemperature)
		{
			...
			int num2 = Math.Max((outputStack.Collectible.CombustibleProps != null) ? outputStack.Collectible.CombustibleProps.MaxTemperature : 0, (outputStack.ItemAttributes?["maxTemperature"] != null) ? outputStack.ItemAttributes["maxTemperature"].AsInt() : 0);
			if (num2 > 0)
			{
	...
	public void igniteWithFuel(IItemStack stack)
	{
		...
		maxTemperature = (int)((float)combustibleProps.BurnTemperature * HeatModifier);
	...
	{
		AssetLocation blockCode = base.Block.CodeWithVariant("burnstate", state);
	...
	private void UpdateRenderer()
	{
		...
		ItemStack itemStack = ((inputStack == null) ? outputStack : inputStack);
		if (renderer.ContentStack != null && renderer.contentStackRenderer != null && itemStack?.Collectible is IInFirepitRendererSupplier && renderer.ContentStack.Equals(Api.World, itemStack, GlobalConstants.IgnoredStackAttributes))
		{
			return;
		}
		...
		{
			IInFirepitRenderer rendererWhenInFirepit = (itemStack?.Collectible as IInFirepitRendererSupplier).GetRendererWhenInFirepit(itemStack, this, itemStack == outputStack);
			if (rendererWhenInFirepit != null)
			{
				renderer.SetChildRenderer(itemStack, rendererWhenInFirepit);
				return;
			}
		}
		InFirePitProps renderProps = GetRenderProps(itemStack);
		if (itemStack?.Collectible != null && !(itemStack?.Collectible is IInFirepitMeshSupplier) && renderProps != null)
		{
			renderer.SetContents(itemStack, renderProps.Transform);
		}
		else
		{
	...
	public CombustibleProperties getCombustibleOpts(int slotid)
	{
		...
		return itemSlot.Itemstack.Collectible.CombustibleProps;
	}
	...
	public override void OnStoreCollectibleMappings(Dictionary<int, AssetLocation> blockIdMapping, Dictionary<int, AssetLocation> itemIdMapping)
	{
		...
		foreach (ItemSlot itemSlot in cookingSlots)
		{
			if (itemSlot.Itemstack != null)
			{
				...
				itemSlot.Itemstack.Collectible.OnStoreCollectibleMappings(Api.World, itemSlot, blockIdMapping, itemIdMapping);
			}
		}
	}
	...
	public override bool OnTesselation(ITerrainMeshPool mesher, ITesselatorAPI tesselator)
	{
		...
		string text = base.Block.Variant["burnstate"];
		...
		mesher.AddMeshData(getOrCreateMesh(text, contentstate));
		return true;
	}
	...
	private MeshData getContentMesh(ItemStack contentStack, ITesselatorAPI tesselator)
	{
		...
		if (contentStack.Collectible is IInFirepitMeshSupplier)
		{
			...
			MeshData meshWhenInFirepit = (contentStack.Collectible as IInFirepitMeshSupplier).GetMeshWhenInFirepit(contentStack, Api.World, Pos, ref firepitModel);
	...
	{
		if (contentStack != null && contentStack.ItemAttributes?.KeyExists("inFirePitProps") == true)
		{
	...
	public MeshData getOrCreateMesh(string burnstate, string contentstate)
	{
		...
		if (!orCreate.TryGetValue(text, out var value))
		{
			...
			_ = new MeshData[17];
			((ICoreClientAPI)Api).Tesselator.TesselateShape(block, Shape.TryGet(Api, "shapes/block/wood/firepit/" + text + ".json"), out value);
		}
		return value;
	}
