...
public class GuiHandbookCommandPage : GuiHandbookPage
{
	...
	{
		float num = (float)GuiElement.scaled(5.0);
		if (Texture == null)
		{
			Recompose(capi);
		}
		capi.Render.Render2DTexturePremultipliedAlpha(Texture.TextureId, x + (double)num, y + (double)num - GuiElement.scaled(3.0), Texture.Width, Texture.Height);
	}
