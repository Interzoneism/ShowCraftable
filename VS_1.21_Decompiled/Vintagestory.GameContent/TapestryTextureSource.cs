...
public class TapestryTextureSource : ITexPositionSource
{
	...
	public TextureAtlasPosition this[string textureCode]
	{
		get
		{
			...
			{
				texturePath = new AssetLocation("block/stone/rock/granite1");
			}
			else if (textureCode == "carving")
			{
				texturePath = new AssetLocation("block/stone/wallcarving/" + type);
			}
			else
			{
				texturePath = new AssetLocation("block/cloth/tapestry/" + type);
			}
			AssetLocation assetLocation = texturePath.Clone();
			AssetLocation rotLoc = null;
			if (rotten)
			{
				rotLoc = new AssetLocation("block/cloth/tapestryoverlay/rotten" + rotVariant);
				assetLocation.Path = assetLocation.Path + "++" + rotLoc.Path;
			}
			capi.BlockTextureAtlas.GetOrInsertTexture(assetLocation, out var _, out var texPos, delegate
			{
