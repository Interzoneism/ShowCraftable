...
public class ModSystemOreMap : ModSystem
{
	...
	{
		if (api.ModLoader.GetModSystem<WorldMapManager>().MapLayers.FirstOrDefault((MapLayer ml) => ml is OreMapLayer) is OreMapLayer oreMapLayer)
		{
			oreMapLayer.Delete(fromPlayer, packet.Index);
		}
	}
	...
	{
		if (capi.ModLoader.GetModSystem<WorldMapManager>().MapLayers.FirstOrDefault((MapLayer ml) => ml is OreMapLayer) is OreMapLayer oreMapLayer)
		{
			oreMapLayer.ownPropickReadings.Add(reading);
			oreMapLayer.RebuildMapComponents();
		}
	}
	...
	public void DidProbe(PropickReading results, IServerPlayer splr)
	{
		if (api.ModLoader.GetModSystem<WorldMapManager>().MapLayers.FirstOrDefault((MapLayer ml) => ml is OreMapLayer) is OreMapLayer oreMapLayer)
		{
			oreMapLayer.getOrLoadReadings(splr).Add(results);
		}
	}
}
