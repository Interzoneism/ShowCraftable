...
public class ResonatorRenderer : IRenderer, IDisposable
{
	...
	public void OnRenderFrame(float deltaTime, EnumRenderStage stage)
	{
		...
		{
			long inWorldEllapsedMilliseconds = api.InWorldEllapsedMilliseconds;
			...
			render.BindTexture2d(api.ItemTextureAtlas.AtlasTextures[0].TextureId);
			float num = -0.5f;
			float y = -0.5f;
			float num2 = -0.5f;
			discPos.X = -0.25f;
			...
			discPos.Z = 0.1375f;
			discRotRad.X = 0f;
			discRotRad.Y = (float)(inWorldEllapsedMilliseconds - updatedTotalMs) / 500f * (float)Math.PI;
			discRotRad.Z = 0f;
			standardShader.NormalShaded = 0;
			standardShader.ModelMatrix = ModelMat.Identity().Translate((double)pos.X - cameraPos.X, (double)pos.Y - cameraPos.Y, (double)pos.Z - cameraPos.Z).Translate(0f - num, 0f, 0f - num2)
