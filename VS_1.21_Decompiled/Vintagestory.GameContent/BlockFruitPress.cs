...
public class BlockFruitPress : Block
{
	...
	public override void OnLoaded(ICoreAPI api)
	{
		...
		api.Event.EnqueueMainThreadTask(delegate
		{
			interactions = ObjectCacheUtil.GetOrCreate(api, "fruitPressInteractions", delegate
			{
				...
				{
					if (collectible is BlockLiquidContainerBase { IsTopOpened: not false, AllowHeldLiquidTransfer: not false, CapacityLitres: <20f })
					{
	...
	public override void OnBlockPlaced(IWorldAccessor world, BlockPos pos, ItemStack byItemStack = null)
	{
		...
		Block block = world.GetBlock(new AssetLocation("fruitpresstop-" + Variant["orientation"]));
	...
	{
		if (world.BlockAccessor.GetBlockEntity(blockSel.Position) is BlockEntityFruitPress blockEntityFruitPress)
		{
			bool result = blockEntityFruitPress.OnBlockInteractStart(byPlayer, blockSel, (blockSel.SelectionBoxIndex == 1) ? EnumFruitPressSection.MashContainer : EnumFruitPressSection.Ground, !RightMouseDown);
			RightMouseDown = true;
			return result;
		}
	...
	public override bool OnBlockInteractStep(float secondsUsed, IWorldAccessor world, IPlayer byPlayer, BlockSelection blockSel)
	{
		if (world.BlockAccessor.GetBlockEntity(blockSel.Position) is BlockEntityFruitPress blockEntityFruitPress)
		{
			return blockEntityFruitPress.OnBlockInteractStep(secondsUsed, byPlayer, (blockSel.SelectionBoxIndex == 1) ? EnumFruitPressSection.MashContainer : EnumFruitPressSection.Ground);
		}
