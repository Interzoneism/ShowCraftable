...
public class ActivityVisualizer : IRenderer, IDisposable
{
	...
	public void LineTo(Vec3d source, Vec3d target, int color = -1)
	{
		...
		labels.Add(new VisualizerLabel
		{
			...
			Texture = Api.Gui.TextTexture.GenTextTexture(lineIndex.ToString() ?? "", CairoFont.WhiteMediumText(), background)
		});
		lineIndex++;
	}
	...
	private void Render2d(float deltaTime)
	{
		...
		foreach (VisualizerLabel label in labels)
		{
			Vec3d vec3d = MatrixToolsd.Project(label.Pos, render.PerspectiveProjectionMat, render.PerspectiveViewMat, render.FrameWidth, render.FrameHeight);
			vec3d.X -= label.Texture.Width / 2;
			vec3d.Y += (double)label.Texture.Height * 1.5;
			render.Render2DTexture(label.Texture.TextureId, (float)vec3d.X, (float)render.FrameHeight - (float)vec3d.Y, label.Texture.Width, label.Texture.Height, 20f);
		}
	}
