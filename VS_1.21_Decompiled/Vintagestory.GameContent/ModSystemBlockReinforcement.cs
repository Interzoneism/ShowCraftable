...
public class ModSystemBlockReinforcement : ModSystem
{
	...
	private TextCommandResult OnCmdGroupRevokeGroup(TextCommandCallingArgs args)
	{
		...
		PlayerGroupMembership playerGroupMembership = serverPlayer.GetGroup(fromChatGroupId);
		if (playerGroupMembership == null)
		{
			return TextCommandResult.Success("Type this command inside the group chat tab that you are a owner of");
		}
		if (playerGroupMembership.Level != EnumPlayerGroupMemberShip.Owner)
		{
			return TextCommandResult.Success("Must be owner of the group to change access flags");
		}
		if (!privGrantsByOwningGroupUid.TryGetValue(fromChatGroupId, out var value))
		{
			value = (privGrantsByOwningGroupUid[fromChatGroupId] = new ReinforcedPrivilegeGrantsGroup());
		}
		return GrantRevokeGroupOwned2Group(serverPlayer, fromChatGroupId, args.Command.Name, groupname, "none", EnumBlockAccessFlags.None, value);
	}
	...
	private TextCommandResult OnCmdGroupGrantGroup(TextCommandCallingArgs args)
	{
		...
		return GrantRevokeGroupOwned2Group(serverPlayer, fromChatGroupId, args.Command.Name, groupname, flagString, flags, value);
	}

	private static EnumBlockAccessFlags GetFlags(string flagString)
	{
		EnumBlockAccessFlags result = EnumBlockAccessFlags.None;
		if (flagString != null)
		{
			if (flagString.ToLowerInvariant() == "use")
			{
				result = EnumBlockAccessFlags.Use;
			}
			if (flagString.ToLowerInvariant() == "all")
			{
				result = EnumBlockAccessFlags.BuildOrBreak | EnumBlockAccessFlags.Use;
			}
		}
		return result;
	}
	...
	private TextCommandResult OnCmdGroupRevoke(TextCommandCallingArgs args)
	{
		...
		if (text == "default")
		{
			value.DefaultGrants = EnumBlockAccessFlags.None;
			SyncPrivData();
			return TextCommandResult.Success("All access revoked for group members");
		}
		GrantRevokeGroupOwned2Player(serverPlayer, fromChatGroupId, args.Command.Name, text, "none", EnumBlockAccessFlags.None, value);
	...
	private TextCommandResult OnCmdGroupGrant(TextCommandCallingArgs args)
	{
		...
		EnumBlockAccessFlags flags = GetFlags(text2);
		if (!privGrantsByOwningGroupUid.TryGetValue(fromChatGroupId, out var value))
		{
			value = (privGrantsByOwningGroupUid[fromChatGroupId] = new ReinforcedPrivilegeGrantsGroup());
		}
		if (text == "default")
		{
			value.DefaultGrants = flags;
			SyncPrivData();
			return TextCommandResult.Success("Default access for group members set to " + text2);
		}
		GrantRevokeGroupOwned2Player(serverPlayer, fromChatGroupId, args.Command.Name, text, text2, flags, value);
		return TextCommandResult.Success();
	}

	private TextCommandResult OnCmdGrantGroup(TextCommandCallingArgs args)
	{
		string forGroupName = args.Parsers[0].GetValue() as string;
		EnumBlockAccessFlags flags = GetFlags(args.Parsers[1].GetValue() as string);
		return SetGroupPrivilege(args.Caller.Player as IServerPlayer, forGroupName, flags);
	}

	private TextCommandResult OnCmdRevokeGroup(TextCommandCallingArgs args)
	{
		string forGroupName = args.Parsers[0].GetValue() as string;
		return SetGroupPrivilege(args.Caller.Player as IServerPlayer, forGroupName, EnumBlockAccessFlags.None);
	}
	...
	private TextCommandResult OnCmdGrant(TextCommandCallingArgs args)
	{
		...
		EnumBlockAccessFlags flags = GetFlags(flagString);
		IServerPlayerData serverPlayerData = null;
		serverPlayerData = coreServerAPI.PlayerData.GetPlayerDataByLastKnownName(name);
		...
		return SetPlayerPrivilege(args.Caller.Player as IServerPlayer, serverPlayerData.PlayerUID, flags);
	}

	private TextCommandResult OnCmdRevoke(TextCommandCallingArgs args)
	{
		string name = args.Parsers[0].GetValue() as string;
		IServerPlayerData playerDataByLastKnownName = (api as ICoreServerAPI).PlayerData.GetPlayerDataByLastKnownName(name);
		if (playerDataByLastKnownName == null)
		{
			return TextCommandResult.Success("No player with such name found or never connected to this server");
		}
		return SetPlayerPrivilege(args.Caller.Player as IServerPlayer, playerDataByLastKnownName.PlayerUID, EnumBlockAccessFlags.None);
	}
	...
	{
		PlayerGroup playerGroupByName = (api as ICoreServerAPI).Groups.GetPlayerGroupByName(groupname);
		...
		if (command == "grant")
		{
			groupGrants.GroupGrants[playerGroupByName.Uid] = flags;
			message = flagString + " access set for group " + groupname;
			SyncPrivData();
		}
		else if (groupGrants.GroupGrants.Remove(playerGroupByName.Uid))
		{
			message = "All access revoked for group " + groupname;
		}
		else
		{
			message = "This group has no access. No action taken.";
			SyncPrivData();
		}
	...
	{
		Dictionary<int, BlockReinforcement> orCreateReinforcmentsAt = getOrCreateReinforcmentsAt(pos);
		if (orCreateReinforcmentsAt == null)
		{
			return false;
		}
		int key = toLocalIndex(pos);
		if (!orCreateReinforcmentsAt.ContainsKey(key))
		{
			errorCode = "notreinforced";
			return false;
		}
		BlockReinforcement blockReinforcement = orCreateReinforcmentsAt[key];
		PlayerGroupMembership playerGroupMembership = forPlayer.GetGroup(blockReinforcement.GroupUid);
		if (blockReinforcement.PlayerUID != forPlayer.PlayerUID && playerGroupMembership == null && (GetAccessFlags(blockReinforcement.PlayerUID, blockReinforcement.GroupUid, forPlayer) & EnumBlockAccessFlags.BuildOrBreak) == 0)
		{
			errorCode = "notownblock";
			return false;
		}
		orCreateReinforcmentsAt.Remove(key);
		SaveReinforcments(orCreateReinforcmentsAt, pos);
		return true;
	}
	...
	public bool IsReinforced(BlockPos pos)
	{
		...
		int key = toLocalIndex(pos);
		return orCreateReinforcmentsAt.ContainsKey(key);
	}
	...
	public bool IsLockedForInteract(BlockPos pos, IPlayer forPlayer)
	{
		...
		int key = toLocalIndex(pos);
		if (orCreateReinforcmentsAt.TryGetValue(key, out var value) && value.Locked && value.PlayerUID != forPlayer.PlayerUID && forPlayer.GetGroup(value.GroupUid) == null)
		{
			if ((GetAccessFlags(value.PlayerUID, value.GroupUid, forPlayer) & EnumBlockAccessFlags.Use) > EnumBlockAccessFlags.None)
			{
				return false;
			}
	...
	public EnumBlockAccessFlags GetAccessFlags(string owningPlayerUid, int owningGroupId, IPlayer forPlayer)
	{
		...
		PlayerGroupMembership playerGroupMembership = forPlayer.GetGroup(owningGroupId);
		if (playerGroupMembership != null)
		{
			return EnumBlockAccessFlags.BuildOrBreak | EnumBlockAccessFlags.Use;
		}
		EnumBlockAccessFlags value = EnumBlockAccessFlags.None;
		if (owningPlayerUid != null && privGrantsByOwningPlayerUid.TryGetValue(owningPlayerUid, out var value2))
		{
			value2.PlayerGrants.TryGetValue(forPlayer.PlayerUID, out value);
			foreach (KeyValuePair<int, EnumBlockAccessFlags> groupGrant in value2.GroupGrants)
			{
				if (forPlayer.GetGroup(groupGrant.Key) != null)
				{
					value |= groupGrant.Value;
				}
			}
		}
		if (owningGroupId != 0 && privGrantsByOwningGroupUid.TryGetValue(owningGroupId, out var value3))
		{
			if (playerGroupMembership != null)
			{
				value3.PlayerGrants.TryGetValue(forPlayer.PlayerUID, out value);
				value |= value3.DefaultGrants;
			}
			foreach (KeyValuePair<int, EnumBlockAccessFlags> groupGrant2 in value3.GroupGrants)
			{
				if (forPlayer.GetGroup(groupGrant2.Key) != null)
				{
					value |= groupGrant2.Value;
				}
			}
		}
		return value;
	...
	public bool TryLock(BlockPos pos, IPlayer byPlayer, string itemCode)
	{
		...
		int key = toLocalIndex(pos);
		if (orCreateReinforcmentsAt.TryGetValue(key, out var value))
		{
			PlayerGroupMembership playerGroupMembership = byPlayer.GetGroup(value.GroupUid);
			bool flag = value.PlayerUID == byPlayer.PlayerUID;
			if (playerGroupMembership != null)
			{
				flag |= playerGroupMembership.Level == EnumPlayerGroupMemberShip.Owner;
				flag |= playerGroupMembership.Level == EnumPlayerGroupMemberShip.Op;
			}
			if (!flag || value.Locked)
			{
				return false;
			}
			value.Locked = true;
			value.LockedByItemCode = itemCode;
			SaveReinforcments(orCreateReinforcmentsAt, pos);
			return true;
		}
		orCreateReinforcmentsAt[key] = new BlockReinforcement
		{
	...
	public BlockReinforcement GetReinforcment(BlockPos pos)
	{
		...
		return orCreateReinforcmentsAt[key];
	}
	...
	public void ConsumeStrength(BlockPos pos, int byAmount)
	{
		...
		if (orCreateReinforcmentsAt.ContainsKey(key))
		{
			orCreateReinforcmentsAt[key].Strength -= byAmount;
			if (orCreateReinforcmentsAt[key].Strength <= 0)
			{
				orCreateReinforcmentsAt.Remove(key);
			}
			SaveReinforcments(orCreateReinforcmentsAt, pos);
	...
	public void ClearReinforcement(BlockPos pos)
	{
		...
		{
			int key = toLocalIndex(pos);
			if (orCreateReinforcmentsAt.ContainsKey(key) && orCreateReinforcmentsAt.Remove(key))
			{
				SaveReinforcments(orCreateReinforcmentsAt, pos);
			}
		}
	...
	public bool StrengthenBlock(BlockPos pos, IPlayer byPlayer, int strength, int forGroupUid = 0)
	{
		...
		{
			BlockReinforcement blockReinforcement = orCreateReinforcmentsAt[key];
			if (blockReinforcement.Strength > 0)
			{
				return false;
			}
			blockReinforcement.Strength = strength;
		}
		else
		{
			string lastGroupname = null;
			if ((api as ICoreServerAPI).Groups.PlayerGroupsById.TryGetValue(forGroupUid, out var value))
			{
				lastGroupname = value.Name;
			}
			orCreateReinforcmentsAt[key] = new BlockReinforcement
			{
	...
	private Dictionary<int, BlockReinforcement> getOrCreateReinforcmentsAt(BlockPos pos)
	{
		...
		byte[] moddata = chunkAtBlockPos.GetModdata("reinforcements");
		if (moddata != null)
		{
			try
			{
				return SerializerUtil.Deserialize<Dictionary<int, BlockReinforcement>>(moddata);
			}
			catch (Exception)
			{
				try
				{
					...
					Dictionary<int, BlockReinforcementOld> dictionary = SerializerUtil.Deserialize<Dictionary<int, BlockReinforcementOld>>(moddata);
					Dictionary<int, BlockReinforcement> dictionary2 = new Dictionary<int, BlockReinforcement>();
					foreach (KeyValuePair<int, BlockReinforcementOld> item in dictionary)
					{
						dictionary2[item.Key] = item.Value.Update();
					}
					SaveReinforcments(dictionary2, pos);
				...
				catch (Exception ex2)
				{
					...
					api.World.Logger.VerboseDebug(LoggerBase.CleanStackTrace(ex2.ToString()));
				}
	...
	{
		if (!privGrantsByOwningPlayerUid.TryGetValue(owningPlayer.PlayerUID, out var value))
		{
			value = new ReinforcedPrivilegeGrants();
			privGrantsByOwningPlayerUid[owningPlayer.PlayerUID] = value;
		}
		string message;
		if (access == EnumBlockAccessFlags.None)
		{
			message = ((!value.PlayerGrants.Remove(forPlayerUid)) ? Lang.Get("No action taken. Player does not have any privilege to your reinforced blocks.") : Lang.Get("Ok, privilege revoked from player."));
		}
		else
		{
			value.PlayerGrants[forPlayerUid] = access;
			message = Lang.Get("Ok, Privilege for player set.");
		}
		SyncPrivData();
		return TextCommandResult.Success(message);
	}
	...
	public TextCommandResult SetGroupPrivilege(IServerPlayer owningPlayer, string forGroupName, EnumBlockAccessFlags access)
	{
		...
		PlayerGroup playerGroupByName = (api as ICoreServerAPI).Groups.GetPlayerGroupByName(forGroupName);
		if (playerGroupByName == null)
		{
		...
		if (access == EnumBlockAccessFlags.None)
		{
			message = ((!value.GroupGrants.Remove(playerGroupByName.Uid)) ? Lang.Get("No action taken. Group does not have any privilege to your reinforced blocks.") : Lang.Get("Ok, privilege revoked from group."));
		}
		else
		{
			value.GroupGrants[playerGroupByName.Uid] = access;
			message = Lang.Get("Ok, Privilege for group set.");
		}
		SyncPrivData();
		return TextCommandResult.Success(message);
	}
