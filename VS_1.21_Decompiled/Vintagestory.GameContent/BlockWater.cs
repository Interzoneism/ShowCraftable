...
public class BlockWater : BlockForFluidsLayer, IBlockFlowing
{
	...
	public override void OnGroundIdle(EntityItem entityItem)
	{
		...
		{
			Vec3d xYZ = entityItem.ServerPos.XYZ;
			...
			entityItem.World.SpawnCubeParticles(xYZ, entityItem.Itemstack, 0.75f, Math.Min(100, (int)(2f * num)), 0.45f);
			entityItem.World.PlaySoundAt(new AssetLocation("sounds/environment/smallsplash"), (float)xYZ.X, (float)xYZ.Y, (float)xYZ.Z);
			if (api.World.BlockAccessor.GetBlockEntity(xYZ.AsBlockPos) is BlockEntityFarmland blockEntityFarmland)
			{
				blockEntityFarmland.WaterFarmland((float)Height / 6f, waterNeightbours: false);
				blockEntityFarmland.MarkDirty(redrawOnClient: true);
			}
		}
	...
	public override bool CanPlaceBlock(IWorldAccessor world, IPlayer byPlayer, BlockSelection blockSel, ref string failureCode)
	{
		...
		if (block.DisplacesLiquids(world.BlockAccessor, blockSel.Position) && !block.IsReplacableBy(this))
		{
