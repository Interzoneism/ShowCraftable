...
public class ItemBook : ItemRollable
{
	...
	{
		string text = itemStack.Attributes.GetString("title");
	...
	public override void GetHeldItemInfo(ItemSlot inSlot, StringBuilder dsc, IWorldAccessor world, bool withDebugInfo)
	{
		...
		string text = inSlot.Itemstack.Attributes.GetString("signedby");
		string text2 = inSlot.Itemstack.Attributes.GetString("transcribedby");
		if (text != null)
		{
			dsc.AppendLine(Lang.Get("Written by {0}", (text.Length == 0) ? Lang.Get("Unknown author") : text));
		}
		if (text2 != null && text2.Length > 0)
		{
			dsc.AppendLine(Lang.Get("Transcribed by {0}", (text2.Length == 0) ? Lang.Get("Unknown author") : text2));
		}
	}
