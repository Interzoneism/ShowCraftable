...
public class TutorialStepReceive : TutorialStepGeneric
{
	...
	public override RichTextComponentBase[] GetText(CairoFont font)
	{
		...
		_ = receiveType;
		string text = Lang.Get(base.text, (rwatcher.QuantityAchieved >= rwatcher.QuantityGoal) ? rwatcher.QuantityGoal : (rwatcher.QuantityGoal - rwatcher.QuantityAchieved));
		text = Lang.Get("tutorialstep-numbered", index + 1, text);
		return VtmlUtil.Richtextify(capi, text, font);
	}
