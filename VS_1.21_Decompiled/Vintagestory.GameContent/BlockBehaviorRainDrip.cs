...
public class BlockBehaviorRainDrip : BlockBehavior
{
	...
	public override bool ShouldReceiveClientParticleTicks(IWorldAccessor world, IPlayer byPlayer, BlockPos pos, ref EnumHandling handling)
	{
		...
		int rainMapHeightAt = world.BlockAccessor.GetRainMapHeightAt(pos);
		if (rainMapHeightAt <= pos.Y || (rainMapHeightAt <= pos.Y + 1 && world.BlockAccessor.GetBlockAbove(pos, 1, 1).HasBehavior<BlockBehaviorRainDrip>()) || (rainMapHeightAt <= pos.Y + 2 && world.BlockAccessor.GetBlockAbove(pos, 2, 1).HasBehavior<BlockBehaviorRainDrip>()))
		{
