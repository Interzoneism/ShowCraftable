...
public class BlockBomb : Block, IIgnitable
{
	...
	public void OnTryIgniteBlockOver(EntityAgent byEntity, BlockPos pos, float secondsIgniting, ref EnumHandling handling)
	{
		if (!(secondsIgniting < 0.7f))
		{
			...
			if (player != null)
			{
				(player.Entity.World.BlockAccessor.GetBlockEntity(pos) as BlockEntityBomb)?.OnIgnite(player);
			}
		}
	}
	...
	{
		(world.BlockAccessor.GetBlockEntity(pos) as BlockEntityBomb)?.OnBlockExploded(pos, ignitedByPlayerUid);
	}
