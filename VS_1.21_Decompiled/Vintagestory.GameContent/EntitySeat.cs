...
public abstract class EntitySeat : IMountableSeat
{
	...
	public virtual void DidUnmount(EntityAgent entityAgent)
	{
		if (Passenger != null)
		{
			if (Passenger.Properties?.Client.Renderer is EntityShapeRenderer entityShapeRenderer)
			{
				entityShapeRenderer.xangle = 0f;
				entityShapeRenderer.yangle = 0f;
				entityShapeRenderer.zangle = 0f;
			}
