...
public class BlockBloomery : Block, IIgnitable
{
	...
	{
		if (!(api.World.BlockAccessor.GetBlockEntity(blockSelection.Position) is BlockEntityBloomery blockEntityBloomery) || wi.Itemstacks.Length == 0)
		{
			return null;
		}
		List<ItemStack> list = new List<ItemStack>();
	...
	public override bool OnBlockInteractStart(IWorldAccessor world, IPlayer byPlayer, BlockSelection blockSel)
	{
		...
		if (itemstack != null && itemstack.Class == EnumItemClass.Block && itemstack.Collectible.Code.PathStartsWith("bloomerychimney"))
		{
		...
		if (world.BlockAccessor.GetBlockEntity(blockSel.Position) is BlockEntityBloomery blockEntityBloomery)
		{
			if (itemstack == null)
			{
				return true;
	...
	public override void OnBlockBroken(IWorldAccessor world, BlockPos pos, IPlayer byPlayer, float dropQuantityMultiplier = 1f)
	{
		...
		if (block.Code.Path == "bloomerychimney")
		{
