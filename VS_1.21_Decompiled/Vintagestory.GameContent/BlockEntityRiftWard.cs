...
public class BlockEntityRiftWard : BlockEntity
{
	...
	private int protectionDistance = 40;
	...
	public override void Initialize(ICoreAPI api)
	{
		...
		protectionDistance = base.Block.Attributes?["protectionDistance"].AsInt(40) ?? 40;
	...
	private void OnServerTick(float dt)
	{
		if (On)
		{
			double num = Api.World.Calendar.TotalDays - lastUpdateTotalDays;
			fuelDays -= num;
			MarkDirty();
		}
		if (!HasFuel)
	...
	public bool OnInteract(BlockSelection blockSel, IPlayer byPlayer)
	{
		...
		JsonObject itemAttributes = activeHotbarSlot.Itemstack.ItemAttributes;
		if (itemAttributes != null && itemAttributes.IsTrue("riftwardFuel") && fuelDays < 0.5)
		{
			fuelDays += activeHotbarSlot.Itemstack.ItemAttributes["rifwardfuelDays"].AsDouble(14.0);
	...
	{
		if (On && sapi.World.Rand.NextDouble() <= 0.95 && rift.Position.HorizontalSquareDistanceTo((double)Pos.X + 0.5, (double)Pos.Z + 0.5) < (float)(protectionDistance * protectionDistance))
		{
			rift.Size = 0f;
			riftsBlocked++;
			MarkDirty();
