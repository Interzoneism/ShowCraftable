...
public class ModSystemResoArchiveCommands : ModSystem
{
	...
	{
		BlockPos blockPos = sapi.ObjectCache.Get("weStartMarker-" + args.Caller.Player.PlayerUID) as BlockPos;
		BlockPos blockPos2 = sapi.ObjectCache.Get("weEndMarker-" + args.Caller.Player.PlayerUID) as BlockPos;
		...
		int foundtotal = 0;
		sapi.World.BlockAccessor.WalkBlocks(blockPos, blockPos2, delegate(Block block, int x, int y, int z)
		{
	...
	private TextCommandResult onSetMicroBlockName(TextCommandCallingArgs args)
	{
		...
		string blockName = args[1] as string;
		BlockEntityMicroBlock blockEntity = sapi.World.BlockAccessor.GetBlockEntity<BlockEntityMicroBlock>(asBlockPos);
		if (blockEntity == null)
		{
	...
	{
		BlockPos asBlockPos = (args[0] as Vec3d).AsBlockPos;
		BlockEntityMusicTrigger blockEntity = sapi.World.BlockAccessor.GetBlockEntity<BlockEntityMusicTrigger>(asBlockPos);
		if (blockEntity == null)
		{
			return TextCommandResult.Error("Selected block is not a music trigger");
		}
		blockEntity.musicTrackLocation = new AssetLocation(args[1] as string);
		blockEntity.MarkDirty(redrawOnClient: true);
	...
	private TextCommandResult onMusicTriggerShowArea(TextCommandCallingArgs args)
	{
		...
		musicAreaPreview(blockEntity, args, show: true);
	...
	private TextCommandResult onModMusicTriggerArea(TextCommandCallingArgs args)
	{
		...
		int num = (int)args[1];
		if (blockEntity.areas.Length <= num)
		{
			return TextCommandResult.Error("No such area at this index");
		}
		Cuboidi cuboidi = new Cuboidi((int)args[2], (int)args[3], (int)args[4], (int)args[5], (int)args[6], (int)args[7]);
		blockEntity.areas[num] = cuboidi;
		blockEntity.MarkDirty(redrawOnClient: true);
		musicAreaPreview(blockEntity, args, show: true);
	...
	{
		BlockPos asBlockPos = (args[0] as Vec3d).AsBlockPos;
		if (args.Caller.Player != null && asBlockPos == null)
		{
		...
		Cuboidi value = new Cuboidi((int)args[1], (int)args[2], (int)args[3], (int)args[4], (int)args[5], (int)args[6]);
		blockEntity.areas = (blockEntity.areas ?? Array.Empty<Cuboidi>()).Append(value);
		blockEntity.MarkDirty(redrawOnClient: true);
		musicAreaPreview(blockEntity, args, show: true);
	...
	private TextCommandResult onRemoveMusicTriggerArea(TextCommandCallingArgs args)
	{
		...
		List<Cuboidi> list = new List<Cuboidi>(blockEntity.areas);
		int num = (int)args[1];
		if (num >= list.Count)
		{
		...
		list.RemoveAt(num);
		blockEntity.areas = list.ToArray();
		blockEntity.MarkDirty(redrawOnClient: true);
		musicAreaPreview(blockEntity, args, show: true);
	...
	private void musicAreaPreview(BlockEntityMusicTrigger bec, TextCommandCallingArgs args, bool show)
	{
		...
		if (show)
		{
			...
			foreach (Cuboidi cuboidi in areas)
			{
				list.Add(bec.Pos.AddCopy(cuboidi.MinX, cuboidi.MinY, cuboidi.MinZ));
				list.Add(bec.Pos.AddCopy(cuboidi.MaxX, cuboidi.MaxY, cuboidi.MaxZ));
			}
		}
		sapi.World.HighlightBlocks(args.Caller.Player, 1292, list, EnumHighlightBlocksMode.Absolute, EnumHighlightShape.Cubes);
	}

	private TextCommandResult onSetTranslateable(TextCommandCallingArgs args)
	{
		BlockPos blockPos = ((args.Caller.Player == null) ? args.Caller.Pos.AsBlockPos : args.Caller.Player?.CurrentBlockSelection?.Position);
		if (args.Caller.Player != null && blockPos == null)
		{
			return TextCommandResult.Error("Need to look at a block");
		}
		if (!(sapi.World.BlockAccessor.GetBlockEntity(blockPos) is BlockEntitySign blockEntitySign))
		{
			return TextCommandResult.Error("Selected block is not sign block");
		}
		bool flag = (bool)args[0];
		blockEntitySign.Translateable = flag;
		return TextCommandResult.Success("Translatable " + (flag ? "enabled" : "disabled"));
	}
	...
	private TextCommandResult onSetLoreCode(TextCommandCallingArgs args)
	{
		...
		BEBehaviorClutterBookshelfWithLore bEBehaviorClutterBookshelfWithLore = sapi.World.BlockAccessor.GetBlockEntity(blockPos)?.GetBehavior<BEBehaviorClutterBookshelfWithLore>();
		if (bEBehaviorClutterBookshelfWithLore == null)
		{
			return TextCommandResult.Error("Selected block is not a bookshelf with lore");
		}
		bEBehaviorClutterBookshelfWithLore.LoreCode = args[0] as string;
		bEBehaviorClutterBookshelfWithLore.Blockentity.MarkDirty(redrawOnClient: true);
	...
	private TextCommandResult onSetCmd(TextCommandCallingArgs args, bool on)
	{
		...
		BEBehaviorJonasHydraulicPump bEBehaviorJonasHydraulicPump = sapi.World.BlockAccessor.GetBlockEntity(blockPos)?.GetBehavior<BEBehaviorJonasHydraulicPump>();
		if (bEBehaviorJonasHydraulicPump == null)
		{
		...
		if (on)
		{
			bEBehaviorJonasHydraulicPump.oncommands = text;
		}
		else
		{
			bEBehaviorJonasHydraulicPump.offcommands = text;
		}
		bEBehaviorJonasHydraulicPump.Blockentity.MarkDirty(redrawOnClient: true);
		if (text == null || text.Length == 0)
		{
			return TextCommandResult.Success((on ? "On" : "Off") + " Command cleared.");
		}
		return TextCommandResult.Success((on ? "On" : "Off") + " Command " + text.Replace("{", "{{").Replace("}", "}}") + " set.");
	}
	...
	private TextCommandResult onLampNodeConfig(TextCommandCallingArgs args)
	{
		...
		INetworkedLight networkedLight = sapi.World.BlockAccessor.GetBlockEntity(blockPos)?.GetBehavior<INetworkedLight>();
		if (networkedLight == null)
		{
			return TextCommandResult.Error("Selected block is not a lamp node");
		}
		networkedLight.setNetwork(args[0] as string);
