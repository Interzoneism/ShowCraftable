...
public class TemporalStabilityEffects : ModSystem, IRenderer, IDisposable
{
	...
	public void OnRenderFrame(float deltaTime, EnumRenderStage stage)
	{
		...
		if (rainAndFogActive)
		{
			...
			float num = Math.Min(1f, secondsPassedRainFogMode / 3f);
			rainfogAmbient.AmbientColor.Weight = num;
			rainfogAmbient.FogColor.Weight = num;
			rainfogAmbient.FogDensity.Weight = num;
			float num2 = 40f * num;
			while (num2-- > 0f)
			{
				float x = (float)capi.World.Rand.NextDouble() * 64f - 32f;
				float y = (float)capi.World.Rand.NextDouble() * 64f - 32f;
				float z = (float)capi.World.Rand.NextDouble() * 64f - 32f;
				Vec3d vec3d = capi.World.Player.Entity.Pos.XYZ.OffsetCopy(x, y, z);
				...
				if (capi.World.BlockAccessor.IsValidPos(pos))
				{
					...
					blackAirParticles.MinPos = vec3d;
