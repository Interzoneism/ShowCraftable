...
{
	private GuiDialogBlockEntityTicker clientDialog;
	...
	{
		Block blockOnSide = Api.World.BlockAccessor.GetBlockOnSide(Pos, facing);
		if (blockOnSide != null)
		{
			try
			{
				...
				blockOnSide.Activate(Api.World, caller2, new BlockSelection(Pos.AddCopy(facing), facing.Opposite, blockOnSide));
			}
			catch (Exception e)
			{
				Api.Logger.Warning("Exception thrown when trying to interact with block {0}:", blockOnSide.Code.ToShortString());
	...
	public override void OnReceivedClientPacket(IPlayer fromPlayer, int packetid, byte[] data)
	{
		...
		{
			EditTickerPacket editTickerPacket = SerializerUtil.Deserialize<EditTickerPacket>(data);
			tickIntervalMs = editTickerPacket.Interval.ToInt(500);
			active = editTickerPacket.Active;
			if (listenerId > 0)
			{
