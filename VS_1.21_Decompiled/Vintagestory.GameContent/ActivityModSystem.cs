...
public class ActivityModSystem : ModSystem
{
	...
	{
		api.ChatCommands.GetOrCreate("dev").BeginSubCommand("aedit").RequiresPrivilege(Privilege.controlserver)
			...
			.HandleWith(delegate(TextCommandCallingArgs args)
			{
				...
				string text = (Debug ? "on" : "off");
				return TextCommandResult.Success("Activity Debugging: " + text);
			})
			.EndSubCommand()
			.EndSubCommand();
	}
