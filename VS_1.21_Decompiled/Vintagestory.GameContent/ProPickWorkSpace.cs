...
public class ProPickWorkSpace
{
	...
	{
		DummyChunk[] array = new DummyChunk[sapi.World.BlockAccessor.MapSizeY / 32];
		int num = posX / 32;
		int num2 = posZ / 32;
		int num3 = posX % 32;
		int num4 = posZ % 32;
		...
		for (int j = 0; j < num5; j++)
		{
			int num6 = j / 32;
			int num7 = j - num6 * 32;
			int index3d = (32 * num7 + num4) * 32 + num3;
			array[num6].Blocks[index3d] = rockStrataGen.rockBlockId;
		}
		GenRockStrataNew genRockStrataNew = rockStrataGen;
		IServerChunk[] chunks = array;
		genRockStrataNew.preLoad(chunks, num, num2);
		GenRockStrataNew genRockStrataNew2 = rockStrataGen;
		chunks = array;
		genRockStrataNew2.genBlockColumn(chunks, num, num2, num3, num4);
		if (depositGen == null)
		{
			int num8 = 100;
			while (depositGen == null && num8-- > 0)
			{
				Thread.Sleep(30);
			}
			if (depositGen == null)
		...
		chunks = array;
		genDeposits.GeneratePartial(chunks, num, num2, 0, 0);
		int[] array2 = new int[num5];
		for (int k = 0; k < num5; k++)
		{
			int num9 = k / 32;
			int num10 = k - num9 * 32;
			int index3d2 = (32 * num10 + num4) * 32 + num3;
			array2[k] = array[num9].Blocks[index3d2];
		}
		return array2;
	}
}
