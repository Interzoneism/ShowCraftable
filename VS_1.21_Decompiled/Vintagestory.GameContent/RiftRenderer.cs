...
public class RiftRenderer : IRenderer, IDisposable
{
	...
	public RiftRenderer(ICoreClientAPI capi, Dictionary<int, Rift> rifts)
	{
		...
		meshref = capi.Render.UploadMesh(quad);
		matrixf = new Matrixf();
	...
	public void OnRenderFrame(float deltaTime, EnumRenderStage stage)
	{
		...
		EntityBehaviorTemporalStabilityAffected behavior = capi.World.Player.Entity.GetBehavior<EntityBehaviorTemporalStabilityAffected>();
		if (behavior != null)
		{
		...
		if (modsys.nearestRifts.Length != 0)
		{
			...
			float num = Math.Max(0f, GameMath.Sqrt(pos.SquareDistanceTo(rift.Position)) - 1f - rift.Size / 2f);
			float num2 = Math.Max(0f, 1f - num / 3f);
			GlobalConstants.GuiGearRotJitter = ((capi.World.Rand.NextDouble() < 0.25) ? (num2 * ((float)capi.World.Rand.NextDouble() - 0.5f) / 1f) : 0f);
			capi.ModLoader.GetModSystem<SystemTemporalStability>().modGlitchStrength = Math.Min(1f, num2 * 1.3f);
			if (behavior != null)
			{
		...
		float value = 200f + (float)GameMath.Sin((double)capi.InWorldEllapsedMilliseconds / 24000.0) * 100f;
		prog.Uniform("counterSmooth", value);
		prog.Uniform("invFrameSize", new Vec2f(1f / (float)frameWidth, 1f / (float)frameHeight));
		int num3 = 0;
		cnt = (cnt + 1) % 3;
		foreach (Rift value2 in rifts.Values)
		{
			if (cnt == 0)
			{
				value2.Visible = capi.World.BlockAccessor.GetChunkAtBlockPos((int)value2.Position.X, (int)value2.Position.Y, (int)value2.Position.Z) != null;
			}
			num3++;
			matrixf.Identity();
			float num4 = (float)(value2.Position.X - pos.X);
			float num5 = (float)(value2.Position.Y - pos.Y);
			float num6 = (float)(value2.Position.Z - pos.Z);
			matrixf.Translate(num4, num5, num6);
			...
			float nowSize = value2.GetNowSize(capi);
			matrixf.Scale(nowSize, nowSize, nowSize);
			prog.UniformMatrix("modelViewMatrix", matrixf.Values);
			prog.Uniform("worldPos", new Vec4f(num4, num5, num6, 0f));
			prog.Uniform("riftIndex", num3);
			capi.Render.RenderMesh(meshref);
			if (num4 * num4 + num5 * num5 + num6 * num6 < 1600f)
			{
				Vec3d position = value2.Position;
				capi.World.SpawnParticles(0.1f, ColorUtil.ColorFromRgba(10, 35, 58, 128), position, position, new Vec3f(-0.125f, -0.125f, -0.125f), new Vec3f(0.125f, 0.125f, 0.125f), 5f, 0f, (0.0625f + (float)capi.World.Rand.NextDouble() * 0.25f) / 2f);
			}
		}
