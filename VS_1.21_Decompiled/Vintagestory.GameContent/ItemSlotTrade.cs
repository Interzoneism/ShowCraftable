...
public class ItemSlotTrade : ItemSlot
{
	...
	public override bool DrawUnavailable
	{
		get
		{
			ResolvedTradeItem tradeItem = TradeItem;
			if (tradeItem == null)
			{
				return false;
			}
	...
	{
		TradeItem = tradeItem;
		itemstack = tradeItem.Stack;
	}

	public ItemSlotTrade(InventoryBase inventory, bool isBuyingSlot = false)
		: base(inventory)
	{
		IsBuyingSlot = isBuyingSlot;
	}
