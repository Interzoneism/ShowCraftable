...
public class BlockEntityLocustNest : BlockEntitySpawner
{
	...
	public override void OnBlockPlaced(ItemStack byItemStack = null)
	{
		...
		float num = Math.Min(1f, 1.5f - (float)Pos.Y / (0.36f * (float)Api.World.SeaLevel));
		string text = "locust-bronze";
		...
		Data = new BESpawnerData
		{
			...
			SpawnOnlyAfterImport = (byItemStack?.Attributes?.GetBool("spawnOnlyAfterImport") == true),
	...
	public override void OnReceivedClientPacket(IPlayer fromPlayer, int packetid, byte[] bytes)
	{
		...
		int num = (Api as ICoreServerAPI).World.Rand.Next(entityCodes.Length);
		EntityProperties entityType = Api.World.GetEntityType(new AssetLocation(entityCodes[num]));
		if (entityType == null)
		{
			return;
		}
		...
		for (int i = 0; i < 15; i++)
		{
			vec3d.Set(Pos).Add(-0.5 + Api.World.Rand.NextDouble(), -1.0, -0.5 + Api.World.Rand.NextDouble());
			if (!collisionTester.IsColliding(Api.World.BlockAccessor, entityBoxRel, vec3d, alsoCheckTouch: false))
			{
				...
				DoSpawn(entityType, vec3d, herdId);
				flag = true;
				break;
			}
		}
