...
public class AttachableInteractionHelp
{
	public static WorldInteraction[] GetOrCreateInteractionHelp(ICoreAPI api, EntityBehaviorAttachable eba, WearableSlotConfig[] wearableSlots, int slotIndex, ItemSlot slot)
	{
		...
		List<ItemStack> orCreate = ObjectCacheUtil.GetOrCreate(api, key, delegate
		{
			List<ItemStack> list = new List<ItemStack>();
			foreach (CollectibleObject collectible in api.World.Collectibles)
			{
				if (collectible.CreativeInventoryTabs.Length != 0 || collectible.CreativeInventoryStacks != null)
				{
					IAttachableToEntity attachableToEntity = IAttachableToEntity.FromCollectible(collectible);
					if (attachableToEntity != null)
					{
						if (collectible.CreativeInventoryStacks != null)
						{
							CreativeTabAndStackList[] creativeInventoryStacks = collectible.CreativeInventoryStacks;
							for (int i = 0; i < creativeInventoryStacks.Length; i++)
							{
								JsonItemStack[] stacks = creativeInventoryStacks[i].Stacks;
								foreach (JsonItemStack jsonItemStack in stacks)
								{
		...
		return new WorldInteraction[1]
		{
			new WorldInteraction
			{
				...
				Itemstacks = (slot.Empty ? orCreate.ToArray() : null),
