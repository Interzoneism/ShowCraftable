...
public class BEBehaviorClutterBookshelfWithLore : BEBehaviorClutterBookshelf
{
	...
	public bool OnInteract(IPlayer byPlayer)
	{
		...
		string text = colors[GameMath.MurmurHash3Mod(base.Pos.X, base.Pos.Y, base.Pos.Z, colors.Length)];
		ItemStack itemStack = new ItemStack(Api.World.GetItem(new AssetLocation("lore-book-" + text)));
		itemStack.Attributes.SetString("category", LoreCode);
		if (!byPlayer.InventoryManager.TryGiveItemstack(itemStack))
		{
			EntityPos pos = byPlayer.Entity.Pos;
			Api.World.SpawnItemEntity(itemStack, pos.XYZ);
		}
		Api.World.Logger.Audit("{0} Took 1x{1} from {2} at {3}.", byPlayer.PlayerName, itemStack.Collectible.Code, base.Block.Code, base.Pos);
		LoreCode = null;
	...
	private MeshData genMesh(AssetLocation assetLocation)
	{
		return ObjectCacheUtil.GetOrCreate(Api, "bookshelflorebook-" + Variant + " - " + assetLocation.Path + "-" + rotateX + "-" + base.rotateY + "-" + rotateZ, delegate
		{
			...
			if (Variant == "half")
			{
				modeldata.Translate(-0.5f * GameMath.Sin(base.rotateY), 0f, -0.5f * GameMath.Cos(base.rotateY));
			}
			return modeldata;
		});
	}
