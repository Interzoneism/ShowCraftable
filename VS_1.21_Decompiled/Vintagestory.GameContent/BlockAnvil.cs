...
public class BlockAnvil : Block
{
			...
			return new WorldInteraction[6]
			{
				...
				new WorldInteraction
				{
					...
					GetMatchingStacks = delegate(WorldInteraction wi, BlockSelection bs, EntitySelection es)
					{
						...
						return (blockEntityAnvil?.WorkItemStack != null) ? new ItemStack[1] { blockEntityAnvil.WorkItemStack.Collectible.GetCollectibleInterface<IAnvilWorkable>().GetBaseMaterial(blockEntityAnvil.WorkItemStack) } : null;
					}
				}
			};
		});
	...
	public override void OnDecalTesselation(IWorldAccessor world, MeshData decalMesh, BlockPos pos)
	{
		...
		if (world.BlockAccessor.GetBlockEntity(pos) is BlockEntityAnvil blockEntityAnvil)
		{
	...
	{
		if (blockAccessor.GetBlockEntity(pos) is BlockEntityAnvil blockEntityAnvil)
		{
			Cuboidf[] selectionBoxes = blockEntityAnvil.GetSelectionBoxes(blockAccessor, pos);
			float num = Math.Abs(blockEntityAnvil.MeshAngle * (180f / (float)Math.PI));
	...
	{
		if (world.BlockAccessor.GetBlockEntity(blockSel.Position) is BlockEntityAnvil blockEntityAnvil)
		{
	...
	public override bool DoPlaceBlock(IWorldAccessor world, IPlayer byPlayer, BlockSelection blockSel, ItemStack byItemStack)
	{
		...
		if (num && world.BlockAccessor.GetBlockEntity(blockSel.Position) is BlockEntityAnvil blockEntityAnvil)
		{
