...
public class GuiDialogCommandHandbook : GuiDialogHandbook
{
	...
	public GuiDialogCommandHandbook(ICoreClientAPI capi, OnCreatePagesDelegate createPageHandlerAsync, OnComposePageDelegate composePageHandler)
	...
	protected override GuiTab[] genTabs(out int curTab)
	{
		...
		return new GuiTab[3]
		{
			...
			new HandbookTab
			{
				Name = Lang.Get("Server Commands"),
				CategoryCode = "server"
			},
			new HandbookTab
			{
				Name = Lang.Get("Client Commands"),
				CategoryCode = "client"
			}
		};
	}
