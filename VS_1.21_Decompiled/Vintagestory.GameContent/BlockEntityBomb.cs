...
public class BlockEntityBomb : BlockEntity
{
	...
	{
		int num = (int)Math.Ceiling(BlastRadius);
		Cuboidi cuboidi = new Cuboidi(Pos.AddCopy(-num, -num, -num), Pos.AddCopy(num, num, num));
		List<LandClaim> landClaims = (Api as ICoreServerAPI).WorldManager.LandClaims;
		IPlayer player = Api.World.PlayerByUid(ignitedByPlayerUid);
		for (int i = 0; i < landClaims.Count; i++)
		{
			if (landClaims[i].Intersects(cuboidi))
			{
				return landClaims[i].TestPlayerAccess(player, EnumBlockAccessFlags.BuildOrBreak) != EnumPlayerAccessResult.Denied;
			}
		}
		return true;
	}
	...
	internal void OnBlockExploded(BlockPos pos, string ignitedByPlayerUid)
	{
		if (Api.Side == EnumAppSide.Server)
		{
			...
			if ((!lit || (double)RemainingSeconds > 0.3) && HasPermissionToUse())
			{
