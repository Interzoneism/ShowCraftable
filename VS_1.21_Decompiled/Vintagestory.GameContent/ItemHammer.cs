...
public class ItemHammer : Item
{
	...
	public override void OnHeldActionAnimStart(ItemSlot slot, EntityAgent byEntity, EnumHandInteract type)
	{
		...
		{
			if (blockEntityAnvilPart.TestReadyToMerge())
			{
				startHitAction(slot, byEntity, merge: true);
			}
		}
		else if (byEntity.World.BlockAccessor.GetBlock(blockSelection.Position) is BlockAnvil && blockEntity is BlockEntityAnvil blockEntityAnvil)
		{
	...
	private void startHitAction(ItemSlot slot, EntityAgent byEntity, bool merge)
	{
		...
		byEntity.AnimManager.RegisterFrameCallback(new AnimFrameCallback
		{
			...
			Callback = delegate
			{
				strikeAnvil(byEntity, slot, slot.Itemstack);
			}
		});
	}
	...
	public override bool OnHeldAttackStep(float secondsUsed, ItemSlot slot, EntityAgent byEntity, BlockSelection blockSelection, EntitySelection entitySel)
	{
		...
		if (byEntity.World.BlockAccessor.GetBlockEntity(blockSelection.Position) is BlockEntityAnvilPart blockEntityAnvilPart && !blockEntityAnvilPart.TestReadyToMerge())
		{
			return false;
		}
		string heldTpHitAnimation = GetHeldTpHitAnimation(slot, byEntity);
	...
	protected virtual void strikeAnvil(EntityAgent byEntity, ItemSlot slot, ItemStack strikingItem)
	{
		...
		if (currentBlockSelection == null || strikingItem != player.InventoryManager.ActiveHotbarSlot.Itemstack)
		{
			return;
		}
		...
		{
			if (!(byEntity.World.BlockAccessor.GetBlock(currentBlockSelection.Position) is BlockAnvil) || !(blockEntity is BlockEntityAnvil blockEntityAnvil))
			{
				return;
			}
			if (api.World.Side == EnumAppSide.Client)
			{
				blockEntityAnvil.OnUseOver(player, currentBlockSelection.SelectionBoxIndex);
			}
		}
		slot.Itemstack?.TempAttributes.SetBool("isAnvilAction", value: false);
	}
	...
	private void DrawSplit(Context cr, int x, int y, float width, float height, double[] colordoubles)
	{
		...
		val = (Pattern)new SolidPattern(colordoubles[0], colordoubles[1], colordoubles[2], colordoubles[3]);
		cr.SetSource(val);
		cr.NewPath();
	...
	private void DrawUpset(Context cr, int x, int y, float width, float height, double[] colordoubles, double rot)
	{
		...
		matrix.Translate((double)((float)x + Math.Max(0f, (width - num * num3) / 2f)), (double)((float)y + Math.Max(0f, (height - num2 * num3) / 2f)));
		matrix.Scale((double)num3, (double)num3);
		matrix.Translate((double)(num / 2f), (double)(num2 / 2f));
		matrix.Rotate(rot);
		matrix.Translate((double)((0f - num) / 2f), (double)((0f - num2) / 2f));
		cr.Matrix = matrix;
		cr.Operator = (Operator)2;
		val = (Pattern)new SolidPattern(colordoubles[0], colordoubles[1], colordoubles[2], colordoubles[3]);
		cr.SetSource(val);
