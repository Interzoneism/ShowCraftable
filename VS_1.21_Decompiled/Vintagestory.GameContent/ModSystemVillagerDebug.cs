...
public class ModSystemVillagerDebug : ModSystem
{
	...
	{
		bool flag = args.Parsers[1].IsMissing || (bool)args[1];
		return freeze(args, flag);
	}
	...
	private TextCommandResult proNaked(TextCommandCallingArgs args)
	{
		return OnEach(args, delegate(EntityDressedHumanoid edh, Dictionary<string, WeightedCode[]> pro)
		{
			...
			{
				pro[slotAlloc.Code] = Array.Empty<WeightedCode>();
			}
			edh.LoadOutfitCodes();
	...
	private TextCommandResult proExport(TextCommandCallingArgs args)
	{
		...
		sapi.Network.GetChannel("worldedit").SendPacket<CopyToClipboardPacket>(new CopyToClipboardPacket
		{
			Text = sb.ToString()
		}, new IServerPlayer[1] { args.Caller.Player as IServerPlayer });
		return result;
	}
	...
	{
		EntityDressedHumanoid entityDressedHumanoid = entity as EntityDressedHumanoid;
		if (entityDressedHumanoid.partialRandomOutfitsOverride == null)
		{
			entityDressedHumanoid.partialRandomOutfitsOverride = entity.Properties.Attributes["partialRandomOutfits"].AsObject<Dictionary<string, WeightedCode[]>>();
			if (entityDressedHumanoid.partialRandomOutfitsOverride == null)
			{
				entityDressedHumanoid.partialRandomOutfitsOverride = new Dictionary<string, WeightedCode[]>();
			}
		}
		return entityDressedHumanoid.partialRandomOutfitsOverride;
	}
}
