...
public class TurnAction : EntityActionBase
{
	...
	public override void AddGuiEditFields(ICoreClientAPI capi, GuiComposer singleComposer)
	{
		...
		singleComposer.AddStaticText("Yaw (in degrees)", CairoFont.WhiteDetailText(), elementBounds).AddNumberInput(elementBounds = elementBounds.BelowCopy(0.0, -5.0), null, CairoFont.WhiteDetailText(), "yaw").AddSmallButton("Insert Player Yaw", () => onClickPlayerYaw(capi, singleComposer), elementBounds = elementBounds.FlatCopy().WithFixedPosition(0.0, 0.0).FixedUnder(elementBounds, 2.0), EnumButtonStyle.Small);
	...
	private bool onClickPlayerYaw(ICoreClientAPI capi, GuiComposer singleComposer)
	{
		...
		singleComposer.GetTextInput("yaw").SetValue(Math.Round(GameMath.Mod(pos.Yaw * (180f / (float)Math.PI), 360f), 1).ToString() ?? "");
		return true;
	}
