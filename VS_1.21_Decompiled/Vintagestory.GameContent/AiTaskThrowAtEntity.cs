...
public class AiTaskThrowAtEntity : AiTaskBaseTargetable
{
	...
	public AiTaskThrowAtEntity(EntityAgent entity, JsonObject taskConfig, JsonObject aiConfig)
	...
	public override bool ContinueExecute(float dt)
	{
		...
		if (accum > (float)releaseAtMs / 1000f && !didThrow)
		{
			...
			entityThrownStone.ProjectileStack = new ItemStack(entity.World.GetItem(new AssetLocation("stone-granite")));
			...
			double num3 = Math.Pow(vec3d.SquareDistanceTo(vec3d2), 0.1);
			Vec3d vec3d3 = (vec3d2 - vec3d).Normalize() * GameMath.Clamp(num3 - 1.0, 0.10000000149011612, 1.0);
			if (yawInaccuracy > 0f)
			{
				Random random = entity.World.Rand;
				vec3d3 = vec3d3.RotatedCopy((float)(random.NextDouble() * (double)yawInaccuracy - (double)yawInaccuracy / 2.0));
			}
			entityThrownStone.ServerPos.SetPosWithDimension(entity.ServerPos.BehindCopy(0.21).XYZ.Add(0.0, entity.LocalEyePos.Y, 0.0));
			entityThrownStone.ServerPos.Motion.Set(vec3d3);
			entityThrownStone.Pos.SetFrom(entityThrownStone.ServerPos);
			entityThrownStone.World = entity.World;
			entity.World.SpawnPriorityEntity(entityThrownStone);
		}
