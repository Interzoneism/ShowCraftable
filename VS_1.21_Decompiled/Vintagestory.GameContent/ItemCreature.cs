...
public class ItemCreature : Item
{
	...
	public override void OnBeforeRender(ICoreClientAPI capi, ItemStack itemstack, EnumItemRenderTarget target, ref ItemRenderInfo renderinfo)
	{
		...
		{
			Dictionary<AssetLocation, MultiTextureMeshRef> orCreate = ObjectCacheUtil.GetOrCreate(capi, "itemcreaturemeshrefs", () => new Dictionary<AssetLocation, MultiTextureMeshRef>());
			if (orCreate.TryGetValue(Code, out var value))
			{
				renderinfo.ModelRef = value;
			}
			else
			{
				orCreate[Code] = (renderinfo.ModelRef = CreateOverlaidMeshRef(capi, Shape, stepParentShapes));
			}
		}
	...
	private MultiTextureMeshRef CreateOverlaidMeshRef(ICoreClientAPI capi, CompositeShape cshape, CompositeShape[] stepParentShapes)
	{
		...
		foreach (CompositeShape compositeShape in stepParentShapes)
		{
			Shape shape2 = capi.Assets.TryGet(compositeShape.Base.WithPathPrefixOnce("shapes/").WithPathAppendixOnce(".json"))?.ToObject<Shape>();
			if (shape2 == null)
			{
				capi.Logger.Error("Entity {0} defines a shape overlay {1}, but no such file found. Will ignore.", Code, compositeShape.Base);
				continue;
			}
			...
			shape2.SubclassForStepParenting(texturePrefixCode);
			shape.StepParentShape(shape2, compositeShape.Base.ToShortString(), cshape.Base.ToShortString(), capi.Logger, delegate(string texcode, AssetLocation tloc)
			{
		...
		capi.Tesselator.TesselateShape(meta, shape, out var modeldata);
	...
	{
		Dictionary<AssetLocation, MultiTextureMeshRef> dictionary = ObjectCacheUtil.TryGet<Dictionary<AssetLocation, MultiTextureMeshRef>>(api, "itemcreaturemeshrefs");
		if (dictionary != null)
		{
