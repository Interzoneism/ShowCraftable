...
public class BlockStairs : Block
{
	...
	public override bool TryPlaceBlock(IWorldAccessor world, IPlayer byPlayer, ItemStack itemstack, BlockSelection blockSel, ref string failureCode)
	{
		...
		{
			array[1] = blockSel.Face;
		}
		else
		{
			array[1] = ((blockSel.HitPosition.Y < 0.5 || !hasDownVariant) ? BlockFacing.UP : BlockFacing.DOWN);
		}
		AssetLocation code = CodeWithVariants(new string[2] { "verticalorientation", "horizontalorientation" }, new string[2]
		{
			array[1].Code,
			array[0].Code
		});
	...
	{
		BlockFacing blockFacing = BlockFacing.HORIZONTALS_ANGLEORDER[((360 - angle) / 90 + BlockFacing.FromCode(Variant["horizontalorientation"]).HorizontalAngleIndex) % 4];
	...
	{
		BlockFacing blockFacing = BlockFacing.FromCode(Variant["horizontalorientation"]);
