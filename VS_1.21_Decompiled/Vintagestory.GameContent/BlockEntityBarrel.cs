...
public class BlockEntityBarrel : BlockEntityLiquidContainer
{
	...
	protected void toggleInventoryDialogClient(IPlayer byPlayer)
	{
		if (invDialog == null)
		{
			...
			invDialog.OpenSound = AssetLocation.Create(base.Block.Attributes?["openSound"].AsString() ?? "sounds/block/barrelopen", base.Block.Code.Domain);
			invDialog.CloseSound = AssetLocation.Create(base.Block.Attributes?["closeSound"].AsString() ?? "sounds/block/barrelclose", base.Block.Code.Domain);
			invDialog.TryOpen();
