...
public class ModSystemMeasuringRope : ModSystem, IRenderer, IDisposable
{
	...
	public override void StartClientSide(ICoreClientAPI api)
	{
		...
		pathModelRef = null;
		meshData.AddVertexSkipTex(0f, 0f, 0f);
		meshData.AddIndex(0);
		meshData.AddVertexSkipTex(1f, 1f, 1f);
		meshData.AddIndex(1);
		updateModel = new MeshData(initialiseArrays: false);
		updateModel.xyz = new float[6];
		pathModelRef = capi.Render.UploadMesh(meshData);
	}

	public void OnRenderFrame(float dt, EnumRenderStage stage)
	{
		ItemSlot rightHandItemSlot = capi.World.Player.Entity.RightHandItemSlot;
		if (!(rightHandItemSlot.Itemstack?.Collectible is ItemMeasuringRope))
		{
			return;
		}
		...
		ITreeAttribute attributes = rightHandItemSlot.Itemstack.Attributes;
		bool flag = start != null;
		bool num = attributes.HasAttribute("startX");
		...
		if (num)
		{
			...
			Vec3d other = blockSel?.FullPosition ?? capi.World.Player.Entity.Pos.XYZ;
			requireRedraw |= !end.Equals(other, 0.001);
			end = other;
			...
			{
				double x = attributes.GetDouble("startX");
				double y = attributes.GetDouble("startY");
				double z = attributes.GetDouble("startZ");
				requireRedraw |= !start.Equals(new Vec3d(x, y, z), 0.001);
				start.Set(x, y, z);
				blockstart = new BlockPos(attributes.GetInt("blockX"), attributes.GetInt("blockY"), attributes.GetInt("blockZ"));
			}
		}
		else if (flag)
		{
