...
public class VariableCondition : IActionCondition, IStorableTypedComponent
{
	...
	{
		string text = vas.Entity.Api.ModLoader.GetModSystem<VariablesModSystem>().GetVariable(scope, Name, e) ?? "";
		string text2 = Value ?? "";
		if (text.Length == 0)
		{
			text = "0";
	...
	public void AddGuiEditFields(ICoreClientAPI capi, GuiComposer singleComposer)
	{
		...
		string[] array2 = new string[3] { "entity", "group", "global" };
		ElementBounds elementBounds = ElementBounds.Fixed(0.0, 0.0, 200.0, 25.0);
		singleComposer.AddStaticText("Variable name", CairoFont.WhiteDetailText(), elementBounds).AddDropDown(array2, array2, (int)scope, null, elementBounds = elementBounds.BelowCopy(0.0, -5.0).WithFixedWidth(80.0), CairoFont.WhiteDetailText(), "scope").AddTextInput(elementBounds.RightCopy(5.0).WithFixedWidth(170.0), null, CairoFont.WhiteDetailText(), "name")
			.AddStaticText("Comparison", CairoFont.WhiteDetailText(), elementBounds = elementBounds.BelowCopy(0.0, 10.0))
			.AddDropDown(array, names, Math.Max(0, array.IndexOf(Comparison)), null, elementBounds = elementBounds.BelowCopy(0.0, -5.0), CairoFont.WhiteDetailText(), "comparison")
			.AddStaticText("Value", CairoFont.WhiteDetailText(), elementBounds = elementBounds.BelowCopy(0.0, 10.0))
