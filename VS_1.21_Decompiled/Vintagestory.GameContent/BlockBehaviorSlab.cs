...
{
	public BlockBehaviorSlab(Block block)
		: base(block)
	{
	}
	...
	public override bool TryPlaceBlock(IWorldAccessor world, IPlayer byPlayer, ItemStack itemstack, BlockSelection blockSel, ref EnumHandling handling, ref string failureCode)
	{
		...
		if (!world.BlockAccessor.GetBlock(blockSel.Position).IsReplacableBy(base.block))
		{
			return false;
		}
		Block block = world.BlockAccessor.GetBlock(base.block.CodeWithParts("down"));
		if ((blockSel.HitPosition.Y > 0.5 && blockSel.Face.IsHorizontal) || blockSel.Face == BlockFacing.DOWN)
		{
			block = world.BlockAccessor.GetBlock(base.block.CodeWithParts("up"));
		}
	...
	public override ItemStack[] GetDrops(IWorldAccessor world, BlockPos pos, IPlayer byPlayer, ref float dropQuantityMultiplier, ref EnumHandling handled)
	{
		...
		{
			new ItemStack(world.BlockAccessor.GetBlock(block.CodeWithParts("down")))
		};
	}
	...
	{
		handling = EnumHandling.PreventDefault;
		if (!(block.LastCodePart() == "up"))
		{
			return block.CodeWithParts("up");
		}
		return block.CodeWithParts("down");
	}
}
