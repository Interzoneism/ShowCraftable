...
public class MechNetworkRenderer : IRenderer, IDisposable
{
	...
	public void AddDevice(IMechanicalPowerRenderable device)
	{
		if (device.Shape != null)
		{
			...
			{
				text = device.Block.Attributes?["mechanicalPower"]?["renderer"].AsString("generic");
			}
			int key = device.Shape.GetHashCode() + text.GetHashCode();
			if (!MechBlockRendererByShape.TryGetValue(key, out value))
			{
				object obj = Activator.CreateInstance(RendererByCode[text], capi, mechanicalPowerMod, device.Block, device.Shape);
				MechBlockRenderer.Add((MechBlockRenderer)obj);
				value = (MechBlockRendererByShape[key] = MechBlockRenderer.Count - 1);
			}
			MechBlockRenderer[value].AddDevice(device);
		}
	}
