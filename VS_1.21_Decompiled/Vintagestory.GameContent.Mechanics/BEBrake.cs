...
public class BEBrake : BlockEntity
{
	...
	private MeshData GenOpenedMesh(ITesselatorAPI tesselator, float rotY)
	{
		...
		if (orCreate.TryGetValue(rotY.ToString() ?? "", out var value))
		{
			return value;
		}
		AssetLocation shapePath = AssetLocation.Create("shapes/block/wood/mechanics/brake-stand-opened.json", base.Block.Code.Domain);
		Shape shape = Shape.TryGet(Api, shapePath);
		tesselator.TesselateShape(base.Block, shape, out value, new Vec3f(0f, rotY, 0f));
		return orCreate[rotY.ToString() ?? ""] = value;
	}
