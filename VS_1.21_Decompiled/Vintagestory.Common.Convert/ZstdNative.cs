...
public static class ZstdNative
{
	...
	public static Version Version
	{
		get
		{
			int num = (int)ZSTD_versionNumber();
	...
	{
		string text = RuntimeEnv.OS switch
		{
			OS.Windows => ".dll", 
			OS.Mac => ".dylib", 
			OS.Linux => ".so", 
		...
		{
			if (NativeLibrary.TryLoad(libraryName + text + ".1", assembly, searchPath, out handle))
			{
				return handle;
			}
		...
		if (!NativeLibrary.TryLoad(Path.Combine(AppDomain.CurrentDomain.BaseDirectory, "Lib", libraryName + text), assembly, searchPath, out handle))
		{
			return IntPtr.Zero;
		}
		return handle;
