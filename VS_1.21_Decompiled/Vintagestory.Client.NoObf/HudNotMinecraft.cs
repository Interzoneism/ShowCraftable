...
public class HudNotMinecraft : HudElement
{
	...
	public override void OnGuiOpened()
	{
		...
		TextExtents textExtents = CairoFont.WhiteSmallishText().GetTextExtents(text);
		...
		ElementBounds bounds = ElementBounds.Fixed(47.0, 13.0, 330.0, 40.0);
		ElementBounds bounds2 = ElementBounds.Fixed(EnumDialogArea.LeftTop, 0.0, 0.0, num + 60.0, 50.0).WithFixedAlignmentOffset(10.0, 10.0);
		crossBounds = ElementBounds.Fixed(EnumDialogArea.LeftTop, 0.0, 0.0, 35.0, 35.0).WithFixedPadding(17.0, 17.0);
		crossBounds.WithParent(bounds2);
		crossBounds.CalcWorldBounds();
		base.SingleComposer = capi.Gui.CreateCompo("notminecraftdialog", bounds2).AddShadedDialogBG(ElementBounds.Fill, withTitleBar: false).BeginChildElements()
			.AddStaticText(text, CairoFont.WhiteSmallishText(), bounds)
			.EndChildElements()
			.Compose();
		TryOpen();
	...
	public override void OnRenderGUI(float deltaTime)
	{
		...
		if (IsOpened())
		{
			int num = (int)GuiElement.scaled(22.0);
			capi.Render.RenderItemstackToGui(dummySlot, crossBounds.drawX + crossBounds.InnerWidth / 2.0, crossBounds.drawY + crossBounds.InnerHeight / 2.0, 50.0, num, -1);
