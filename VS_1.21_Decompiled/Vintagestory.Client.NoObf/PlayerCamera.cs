...
public class PlayerCamera : Camera
{
	...
	public void OnBeforeRenderFrame3D(float dt)
	{
		...
		if (!game.IsPaused)
		{
			...
			float num = 1f / 60f;
			double num2 = physicsUnsimulatedSeconds / (double)num;
			...
			if (UpdateCameraPos)
			{
				Vec3d cameraPosOffset = game.EntityPlayer.CameraPosOffset;
				cameraPos.Set(prevPos.X + (curPos.X - prevPos.X) * num2 + (double)CameraShakeStrength * GameMath.Cos(deltaSum * 100.0) / 10.0 + cameraPosOffset.X, prevPos.Y + (curPos.Y - prevPos.Y) * num2 - (double)CameraShakeStrength * GameMath.Cos(deltaSum * 100.0) / 10.0 + cameraPosOffset.Y, prevPos.Z + (curPos.Z - prevPos.Z) * num2 + (double)CameraShakeStrength * GameMath.Sin(deltaSum * 100.0) / 10.0 + cameraPosOffset.Z);
			}
			cameraPos.Y += game.EntityPlayer.Pos.DimensionYAdjustment;
			base.CamSourcePosition.Set(cameraPos.X, cameraPos.Y, cameraPos.Z);
			base.OriginPosition.Set(0.0, 0.0, 0.0);
			Update(dt, game.interesectionTester);
			Vec3d vec3d = cameraPos;
			...
			if ((double)game.shUniforms.playerReferencePos.HorizontalSquareDistanceTo(vec3d.X, vec3d.Z) > 400000000.0)
			{
				game.shUniforms.playerReferencePos.Set((float)vec3d.X, 0.0, (float)vec3d.Z);
			}
			if ((double)game.shUniforms.playerReferencePosForFoam.HorizontalSquareDistanceTo(vec3d.X, vec3d.Z) > 40000.0)
			{
				game.shUniforms.playerReferencePosForFoam.Set((float)vec3d.X, 0.0, (float)vec3d.Z);
			}
			game.shUniforms.PlayerPos.Set(vec3d.SubCopy(game.shUniforms.playerReferencePos));
			game.shUniforms.PlayerPosForFoam.Set(vec3d.SubCopy(game.shUniforms.playerReferencePosForFoam));
		}
	}
