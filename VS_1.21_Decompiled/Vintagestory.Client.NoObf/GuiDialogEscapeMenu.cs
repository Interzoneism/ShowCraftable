...
internal class GuiDialogEscapeMenu : GuiDialog, IGameSettingsHandler, IGuiCompositeHandler
{
	...
	internal void EscapeMenuHome()
	{
		...
		GuiComposer guiComposer2 = guiComposer.AddIf(player != null && player.WorldData?.CurrentGameMode == EnumGameMode.Survival).Execute(delegate
		{
			bposy += 0.45f;
		...
		base.SingleComposer = guiComposer2.AddIf(player2 != null && player2.WorldData?.CurrentGameMode == EnumGameMode.Creative).Execute(delegate
		{
			bposy += 0.45f;
	...
	private bool OnConfirm()
	{
		...
		if (confirmType == 1)
		{
			bool flag = base.SingleComposer.GetSwitch("switch").On;
			game.OpenedToInternet = true;
			EscapeMenuHome();
			game.SendPacketClient(ClientPackets.Chat(GlobalConstants.GeneralChatGroup, "/upnp 1"));
			game.SendPacketClient(ClientPackets.Chat(GlobalConstants.GeneralChatGroup, "/serverconfig advertise " + (flag ? "1" : "0")));
		}
		return true;
	}
