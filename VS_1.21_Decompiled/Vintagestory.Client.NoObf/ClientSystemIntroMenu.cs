...
public class ClientSystemIntroMenu : ClientSystem
{
	...
	private void onEventBus(string eventName, ref EnumHandling handling, IAttribute data)
	{
		...
		{
			bool extendedDebugInfo = game.extendedDebugInfo;
			game.extendedDebugInfo = false;
			capi.Event.RegisterCallback(after15Secs, 15000);
			game.extendedDebugInfo = extendedDebugInfo;
		}
	}
	...
	public void OnRenderGUI(float deltaTime)
	{
		...
		{
			accum += deltaTime;
			float num = GameMath.Clamp(1.5f * accum / 1f, 0f, 1f) - 1f;
			float num2 = (float)(capi.World.Rand.NextDouble() - 0.5) * 6f * RuntimeEnv.GUIScale * pressAccum;
			float num3 = (float)(capi.World.Rand.NextDouble() - 0.5) * 6f * RuntimeEnv.GUIScale * pressAccum;
			capi.Render.Render2DLoadedTexture(slideInTextBoxTexture, num * (float)slideInTextBoxTexture.Width + num2, 80f * RuntimeEnv.GUIScale + num3);
		}
	}
	...
	{
		base.OnKeyDown(args);
		if (slideInTextBoxTexture != null && args.KeyCode == 93)
		{
			args.Handled = true;
			openkeydown = true;
		}
