...
public class HudIngameError : HudElement
{
	...
	private void OnGameTick(float dt)
	{
		if (errorTextActiveMs != 0L)
		{
			...
			{
				float num = Math.Min(0.25f, 1f - (float)(capi.InWorldEllapsedMilliseconds - errorTextActiveMs) / 500f) * RuntimeEnv.GUIScale;
				Composers["ingameerror"].Bounds.absFixedX = x + (double)num * (capi.World.Rand.NextDouble() * 10.0 - 5.0);
				Composers["ingameerror"].Bounds.absFixedY = y + (double)num * (capi.World.Rand.NextDouble() * 10.0 - 5.0);
			}
			else
			{
	...
	public void ComposeGuis()
	{
		...
		ElementBounds bounds = ElementBounds.Fixed(0.0, -155.0, 600.0, 30.0);
		ClearComposers();
		...
		Composers["ingameerror"] = capi.Gui.CreateCompo("ingameerror", elementBounds.FlatCopy()).BeginChildElements(elementBounds).AddTranspHoverText("", font, 600, bounds, "errortext")
			.EndChildElements()
			.Compose();
