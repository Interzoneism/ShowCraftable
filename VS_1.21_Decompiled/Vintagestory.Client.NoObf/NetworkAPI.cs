...
public class NetworkAPI : ClientSystem, IClientNetworkAPI, INetworkAPI
{
	...
	{
		Dictionary<int, NetworkChannel> dictionary = new Dictionary<int, NetworkChannel>();
		Dictionary<int, UdpNetworkChannel> dictionary2 = new Dictionary<int, UdpNetworkChannel>();
		Packet_NetworkChannels serverPacket = packet.NetworkChannels;
		List<NetworkChannel> list = new List<NetworkChannel>(clientchannels.Values);
		List<UdpNetworkChannel> list2 = new List<UdpNetworkChannel>(clientchannelsUdp.Values);
		int i;
		for (i = 0; i < serverPacket.ChannelNamesCount; i++)
		{
			NetworkChannel networkChannel = list.FirstOrDefault((NetworkChannel ch) => ch.ChannelName == serverPacket.ChannelNames[i]);
			if (networkChannel != null)
			{
				list.Remove(networkChannel);
				networkChannel.channelId = serverPacket.ChannelIds[i];
				networkChannel.Connected = true;
				dictionary[serverPacket.ChannelIds[i]] = networkChannel;
			}
			else
			{
				game.Logger.Warning("Improperly configured mod. Server sends me channel name {0}, but no client side mod registered it.", serverPacket.ChannelNames[i]);
			}
		}
		if (list.Count > 0)
		{
			game.Logger.Warning("Client registered {0} network channels ({1}) the server does not know about, may cause issues.", list.Count, string.Join(", ", list.Select((NetworkChannel ch) => ch.channelName)));
			foreach (NetworkChannel item in list)
			{
		...
		int i2;
		for (i2 = 0; i2 < serverPacket.ChannelUdpNamesCount; i2++)
		{
			UdpNetworkChannel udpNetworkChannel = list2.FirstOrDefault((UdpNetworkChannel ch) => ch.ChannelName == serverPacket.ChannelUdpNames[i2]);
			if (udpNetworkChannel != null)
			{
				list2.Remove(udpNetworkChannel);
				udpNetworkChannel.channelId = serverPacket.ChannelUdpIds[i2];
				udpNetworkChannel.Connected = true;
				dictionary2[serverPacket.ChannelUdpIds[i2]] = udpNetworkChannel;
			}
			else
			{
				game.Logger.Warning("Improperly configured mod. Server sends me udp channel name {0}, but no client side mod registered it.", serverPacket.ChannelUdpNames[i2]);
			}
		}
		if (list2.Count > 0)
		{
			game.Logger.Warning("Client registered {0} network udp channels ({1}) the server does not know about, may cause issues.", list2.Count, string.Join(", ", list2.Select((UdpNetworkChannel ch) => ch.channelName)));
			foreach (UdpNetworkChannel item2 in list2)
			{
	...
	private void HandleCustomPacket(Packet_Server packet)
	{
		...
		Packet_CustomPacket customPacket = packet.CustomPacket;
		if (channels.TryGetValue(customPacket.ChannelId, out var value))
		{
			value.OnPacket(customPacket);
		}
	}

	private void HandleCustomUdpPacket(Packet_CustomPacket packet)
	{
		UdpNetworkChannel value;
		if (!serverChannelsReceived)
		{
			earlyUdpPackets.Enqueue(packet);
		}
		else if (channelsUdp.TryGetValue(packet.ChannelId, out value))
		{
	...
	{
		Packet_Client packet_Client = packetClient as Packet_Client;
		byte[] data = game.Serialize(packet_Client);
		game.SendBlockEntityPacket(x, y, z, packet_Client.Id, data);
	}
	...
	public void SendPlayerPositionPacket()
	{
		...
		Packet_EntityPosition entityPositionPacket = ServerPackets.getEntityPositionPacket(game.EntityPlayer.Pos, game.EntityPlayer, 0);
	...
	public void SendPlayerMountPositionPacket(Entity mount)
	{
		...
		Packet_EntityPosition entityPositionPacket = ServerPackets.getEntityPositionPacket(mount.Pos, mount, 0);
		Packet_UdpPacket udpPacket = new Packet_UdpPacket
		{
			Id = 3,
			EntityPosition = entityPositionPacket
	...
	public void SendEntityPacket(long entityid, object packetClient)
	{
		Packet_Client packet_Client = packetClient as Packet_Client;
		byte[] data = game.Serialize(packet_Client);
		game.SendEntityPacket(entityid, packet_Client.Id, data);
	}
	...
	public void SendEntityPacketWithOffset(long entityid, int packetIdOffset, object packetClient)
	{
		Packet_Client packet_Client = packetClient as Packet_Client;
		byte[] data = game.Serialize(packet_Client);
		game.SendEntityPacket(entityid, packet_Client.Id + packetIdOffset, data);
	}
