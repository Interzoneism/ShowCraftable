...
public class SystemRenderPlayerAimAcc : ClientSystem
{
	...
	public void GenAim()
	{
		for (int i = 0; i < 4; i++)
		{
			...
			meshData.Rgba = new byte[8];
			meshData.xyz = new float[6];
			meshData.Indices = new int[2];
			meshData.Uv = new float[4];
			if (i == 0)
			{
				LineMeshUtil.AddLine2D(meshData, 0f, -0.5f, 0f, -1f, -1);
			}
			if (i == 1)
			{
				LineMeshUtil.AddLine2D(meshData, 0f, 0.5f, 0f, 1f, -1);
			...
			if (i == 2)
			{
				LineMeshUtil.AddLine2D(meshData, -1f, 0f, -0.5f, 0f, -1);
			}
			if (i == 3)
			{
				LineMeshUtil.AddLine2D(meshData, 0.5f, 0f, 1f, 0f, -1);
			}
			aimLinesRef[i] = game.Platform.UploadMesh(meshData);
		}
		MeshData meshData2 = new MeshData();
		meshData2.SetMode(EnumDrawMode.Lines);
		meshData2.xyz = new float[48];
		meshData2.Rgba = new byte[64];
		meshData2.Indices = new int[16];
		meshData2.Uv = new float[32];
		int color = ColorUtil.ToRgba(128, 255, 255, 255);
		LineMeshUtil.AddLine2D(meshData2, -0.5f, -0.5f, -0.05f, -0.5f, color);
		LineMeshUtil.AddLine2D(meshData2, -0.5f, -0.5f, -0.5f, -0.05f, color);
		LineMeshUtil.AddLine2D(meshData2, 0.05f, -0.5f, 0.5f, -0.5f, color);
		LineMeshUtil.AddLine2D(meshData2, 0.5f, -0.5f, 0.5f, -0.05f, color);
		LineMeshUtil.AddLine2D(meshData2, 0.5f, 0.05f, 0.5f, 0.5f, color);
		LineMeshUtil.AddLine2D(meshData2, 0.5f, 0.5f, 0.05f, 0.5f, color);
		LineMeshUtil.AddLine2D(meshData2, -0.05f, 0.5f, -0.5f, 0.5f, color);
		LineMeshUtil.AddLine2D(meshData2, -0.5f, 0.5f, -0.5f, 0.05f, color);
		aimRectangleRef = game.Platform.UploadMesh(meshData2);
	}
