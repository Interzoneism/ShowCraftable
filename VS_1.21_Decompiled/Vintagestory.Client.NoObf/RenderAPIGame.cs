...
public class RenderAPIGame : RenderAPIBase
{
		...
		: base(game.Platform)
	{
		...
		useSSBOs = ClientSettings.AllowSSBOs;
		ScreenManager.Platform.UseSSBOs = useSSBOs;
	...
	{
		Vec4f lightRGBSVec4f = game.WorldMap.GetLightRGBSVec4f(posX, posY, posZ);
		...
		standardShader.RgbaLightIn = ((colorMul == null) ? lightRGBSVec4f : lightRGBSVec4f.Mul(colorMul));
	...
	{
		return game.CreateFrameBuffer(intoTexture);
	}
	...
	{
		game.RenderTextureIntoFrameBuffer(atlasTextureId, fromTexture, sourceX, sourceY, sourceWidth, sourceHeight, fb, targetX, targetY, alphaTest);
	}

	public override void DestroyFrameBuffer(FrameBufferRef fb)
	{
		game.DestroyFrameBuffer(fb);
	}

	public override void RenderRectangle(float posX, float posY, float posZ, float width, float height, int color)
	{
		MeshRef vao = whiteRectangleRef;
		Vec4f outVal = new Vec4f();
		game.guiShaderProg.RgbaIn = ColorUtil.ToRGBAVec4f(color, ref outVal);
	...
	{
		MeshData meshData = new MeshData(4, 4, withNormals: false, withUv: false);
		meshData.SetMode(EnumDrawMode.LineStrip);
		int num = 0;
		meshData.AddVertexSkipTex(posX1, posY1, posZ1, color);
		meshData.AddIndex(num++);
		meshData.AddVertexSkipTex(posX2, posY2, posZ2, color);
		meshData.AddIndex(num++);
		MeshRef meshRef = game.api.renderapi.UploadMesh(meshData);
