...
public class ShadowBox
{
	...
	public void update()
	{
		double[] cameraRotationMatrix = getCameraRotationMatrix();
		Mat4d.MulWithVec4(cameraRotationMatrix, FORWARD, forwardVector);
		...
		Vec4d[] array = calculateFrustumVertices(cameraRotationMatrix, forwardVector, centerNear, centerFar);
		bool flag = true;
		Vec4d[] array2 = array;
		foreach (Vec4d vec4d in array2)
		{
			if (flag)
			{
				minX = vec4d.X;
	...
	{
		float num = Math.Min(1f, (float)ClientSettings.FieldOfView / 90f);
		farWidth = (float)(SHADOW_DISTANCE * (double)num);
		nearWidth = camera.ZNear * num;
