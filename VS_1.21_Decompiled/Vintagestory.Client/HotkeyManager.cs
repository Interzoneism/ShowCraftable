...
public class HotkeyManager
{
	...
	public virtual void RegisterDefaultHotKeys()
	{
		...
		RegisterHotKey("pickblock", Lang.Get("Pick block"), EnumMouseButton.Middle, HotkeyType.GUIOrOtherControls);
	...
	{
		foreach (HotKey value in HotKeys.Values)
		{
			value.CurrentMapping = value.DefaultMapping.Clone();
			ClientSettings.Inst.SetKeyMapping(value.Code, value.CurrentMapping);
		}
	}
	...
	{
		foreach (HotKey item in HotKeys.ValuesOrdered)
		{
			if (item.CurrentMapping.KeyCode == keyEventargs.KeyCode && (!keyup || item.TriggerOnUpAlso) && (item.KeyCombinationType != HotkeyType.CreativeTool || player == null || player.WorldData.CurrentGameMode == EnumGameMode.Creative) && (item.KeyCombinationType != HotkeyType.CreativeOrSpectatorTool || player == null || player.WorldData.CurrentGameMode == EnumGameMode.Creative || player.WorldData.CurrentGameMode == EnumGameMode.Spectator) && (!isGlobal || item.IsGlobalHotkey) && (fallBack ? item.FallbackDidPress(keyEventargs, world, player, allowCharacterControls) : item.DidPress(keyEventargs, world, player, allowCharacterControls)) && item.Handler != null)
			{
				keyEventargs.Handled = true;
				item.CurrentMapping.OnKeyUp = keyup;
				if (item.Handler(item.CurrentMapping))
				{
					this.listeners?.Invoke(item.Code, item.CurrentMapping);
					return true;
				}
			}
		}
	...
	public void RegisterHotKey(string code, string name, int keyCode, HotkeyType type = HotkeyType.CharacterControls, bool altPressed = false, bool ctrlPressed = false, bool shiftPressed = false, bool insertFirst = false, bool triggerOnUpAlso = false)
	{
		...
		if (insertFirst)
		{
			HotKeys.Insert(0, code, hotKey);
		}
		else
		{
			HotKeys[code] = hotKey;
		}
		hotKey.SetDefaultMapping();
		if (ClientSettings.KeyMapping.TryGetValue(code, out var value))
		{
			hotKey.CurrentMapping = value;
		}
	}
	...
	public void RegisterHotKey(string code, string name, int keyCode, int? keyCode2, HotkeyType type = HotkeyType.CharacterControls, bool altPressed = false, bool ctrlPressed = false, bool shiftPressed = false)
	{
		...
		if (ClientSettings.KeyMapping.TryGetValue(code, out var value))
		{
			HotKeys[code].CurrentMapping = value;
		}
	}
	...
	public bool OnMouseButton(ClientMain game, EnumMouseButton button, int modifiers, bool buttonDown)
	{
		...
		keyEvent.CtrlPressed = (modifiers & 2) != 0;
		keyEvent.ShiftPressed = (modifiers & 1) != 0;
		keyEvent.AltPressed = (modifiers & 4) != 0;
		keyEvent.CommandPressed = (modifiers & 8) != 0;
		if (TriggerHotKey(keyEvent, game, game.player, game.AllowCharacterControl, !buttonDown))
		{
			return true;
		}
		return false;
