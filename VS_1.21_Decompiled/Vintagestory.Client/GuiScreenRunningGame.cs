...
public class GuiScreenRunningGame : GuiScreen
{
	...
	private void Connect()
	{
		...
		if (singleplayer)
		{
			...
			DummyTcpNetClient dummyTcpNetClient = new DummyTcpNetClient();
			DummyNetwork[] singlePlayerServerNetwork = platform.GetSinglePlayerServerNetwork();
			dummyTcpNetClient.SetNetwork(singlePlayerServerNetwork[0]);
			runningGame.MainNetClient = dummyTcpNetClient;
			DummyUdpNetClient dummyUdpNetClient = new DummyUdpNetClient();
			runningGame.UdpNetClient = dummyUdpNetClient;
			dummyUdpNetClient.SetNetwork(singlePlayerServerNetwork[1]);
		}
		else
		{
			runningGame.Connectdata = connectData;
			TcpNetClient mainNetClient = new TcpNetClient();
			UdpNetClient udpNetClient = new UdpNetClient();
			runningGame.MainNetClient = mainNetClient;
	...
	public override void RenderToPrimary(float dt)
	{
		...
		float sSAA = ClientSettings.SSAA;
		int width = platform.WindowSize.Width;
		int height = platform.WindowSize.Height;
		int width2 = (int)((float)width * sSAA);
		int height2 = (int)((float)height * sSAA);
		platform.GlViewport(0, 0, width2, height2);
	...
	public void ExitOrRedirect(bool isDisconnect = false, string reason = null)
	{
		...
		if (isDisconnect)
		{
			string text = runningGame.disconnectReason ?? "unknown";
			ScreenManager.Platform.Logger.Notification("Exiting current game to disconnected screen, reason: {0}", text);
			ScreenManager.LoadScreen(new GuiScreenDisconnected(text, ScreenManager, ScreenManager.mainScreen));
		}
		else
		{
	...
	private void printWarningsAndEndCapture()
	{
		lock (warningEntriesLock)
		{
			...
			if (warningEntries.Count > 0)
			{
				...
				foreach (LogEntry warningEntry in warningEntries)
				{
					ILogger logger;
					if (warningEntry.Side != EnumAppSide.Server)
					{
					...
					logger?.Log(warningEntry.Logtype, warningEntry.Message, warningEntry.args);
				}
				ScreenManager.Platform.Logger.Warning("===============================================================");
			}
			else
