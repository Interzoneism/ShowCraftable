...
public class ClientPackets
{
	...
	{
		Packet_ClientIdentification identification = new Packet_ClientIdentification
		{
	...
	public static Packet_Client Chat(int groupid, string message, string data = null)
	{
		...
		packet_ChatLine.Message = message;
		packet_ChatLine.Groupid = groupid;
		packet_ChatLine.Data = data;
		return new Packet_Client
		{
			Id = 4,
			Chatline = packet_ChatLine
		};
	}

	public static Packet_Client PingReply()
	{
		Packet_ClientPingReply pingReply = new Packet_ClientPingReply();
		return new Packet_Client
		{
			Id = 2,
			PingReply = pingReply
	...
	{
		Packet_EntityInteraction entityInteraction = new Packet_EntityInteraction
		{
			EntityId = entityId,
	...
	public static Packet_Client BlockInteraction(BlockSelection blockSel, int mode, int type)
	{
		...
		packet_ClientBlockPlaceOrBreak.X = blockSel.Position.X;
		packet_ClientBlockPlaceOrBreak.Y = blockSel.Position.InternalY;
		packet_ClientBlockPlaceOrBreak.Z = blockSel.Position.Z;
		packet_ClientBlockPlaceOrBreak.Mode = mode;
		packet_ClientBlockPlaceOrBreak.BlockType = type;
		packet_ClientBlockPlaceOrBreak.OnBlockFace = blockSel.Face.Index;
		packet_ClientBlockPlaceOrBreak.HitX = CollectibleNet.SerializeDouble(blockSel.HitPosition.X);
		packet_ClientBlockPlaceOrBreak.HitY = CollectibleNet.SerializeDouble(blockSel.HitPosition.Y);
		packet_ClientBlockPlaceOrBreak.HitZ = CollectibleNet.SerializeDouble(blockSel.HitPosition.Z);
		packet_ClientBlockPlaceOrBreak.DidOffset = (blockSel.DidOffset ? 1 : 0);
		packet_ClientBlockPlaceOrBreak.SelectionBoxIndex = blockSel.SelectionBoxIndex;
		return new Packet_Client
		{
			Id = 3,
			BlockPlaceOrBreak = packet_ClientBlockPlaceOrBreak
		};
	}
