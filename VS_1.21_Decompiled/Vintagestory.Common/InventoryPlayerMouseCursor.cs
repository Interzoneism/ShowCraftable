...
public class InventoryPlayerMouseCursor : InventoryBasePlayer
{
	...
	public override void OnItemSlotModified(ItemSlot slot)
	{
		...
		if (wasEmpty && !slot.Empty)
		{
			...
			treeAttribute["itemstack"] = new ItemstackAttribute(slot.Itemstack.Clone());
			treeAttribute["byentityid"] = new LongAttribute((base.Player?.Entity?.EntityId).GetValueOrDefault());
			Api.Event.PushEvent("onitemgrabbed", treeAttribute);
		}
		wasEmpty = slot.Empty;
	}
