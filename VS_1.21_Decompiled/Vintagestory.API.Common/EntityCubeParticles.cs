...
public class EntityCubeParticles : CollectibleParticleProperties
{
	...
	public EntityCubeParticles(IWorldAccessor world, long entityId, Vec3d particlePos, float radius, int quantity, float minScale, float maxScale)
	{
		...
		if (world.Side == EnumAppSide.Client && (world as IClientWorldAccessor).LoadedEntities.TryGetValue(entityId, out var value))
		{
			textureSubId = value.Properties.Client.FirstTexture.Baked.TextureSubId;
		}
	}
	...
	{
		base.Init(api);
		if (textureSubId == 0 && api.Side == EnumAppSide.Client && (api.World as IClientWorldAccessor).LoadedEntities.TryGetValue(entityId, out var value))
		{
			textureSubId = value.Properties.Client.FirstTexture.Baked.TextureSubId;
		}
	}
