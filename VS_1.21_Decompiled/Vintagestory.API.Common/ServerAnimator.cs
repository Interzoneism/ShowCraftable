...
public class ServerAnimator : ClientAnimator
{
	...
	{
		ServerAnimator serverAnimator;
		if (entity is EntityAgent)
		{
			EntityAgent entityag = entity as EntityAgent;
			serverAnimator = new ServerAnimator(() => (double)entityag.Controls.MovespeedMultiplier * entityag.GetWalkSpeedMultiplier(), animations, rootElements, jointsById, entity.AnimManager.TriggerAnimationStopped, requirePosesOnServer);
		}
		else
		{
			serverAnimator = new ServerAnimator(() => 1.0, animations, rootElements, jointsById, entity.AnimManager.TriggerAnimationStopped, requirePosesOnServer);
		}
		serverAnimator.entityForLogging = entity;
