...
public class Packet_EntityAttributeUpdateSerializer
{
	...
	public static Packet_EntityAttributeUpdate DeserializeLengthDelimitedNew(CitoMemoryStream stream)
	{
		...
		DeserializeLengthDelimited(stream, packet_EntityAttributeUpdate);
	...
	public static void Serialize(CitoStream stream, Packet_EntityAttributeUpdate instance)
	{
		...
		{
			Packet_PartialAttribute[] attributes = instance.Attributes;
			int attributesCount = instance.AttributesCount;
			for (int i = 0; i < attributes.Length && i < attributesCount; i++)
			{
				stream.WriteByte(18);
				Packet_PartialAttributeSerializer.SerializeWithSize(stream, attributes[i]);
			}
		}
	}
	...
	public static int GetSize(Packet_EntityAttributeUpdate instance)
	{
		...
		if (instance.Attributes != null)
		{
			...
			{
				int size = Packet_PartialAttributeSerializer.GetSize(instance.Attributes[i]);
