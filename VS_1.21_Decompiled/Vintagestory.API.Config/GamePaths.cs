...
public static class GamePaths
{
	...
	public static void EnsurePathsExist()
	{
		if (!Directory.Exists(Config))
		{
			...
			{
				string folderPath = Environment.GetFolderPath(Environment.SpecialFolder.UserProfile, Environment.SpecialFolderOption.DoNotVerify);
				string folderPath2 = Environment.GetFolderPath(Environment.SpecialFolder.ApplicationData, Environment.SpecialFolderOption.DoNotVerify);
				folderPath2 = Path.Combine(folderPath2, "VintagestoryData");
				if (Config.Equals(folderPath2))
				{
					string text = Path.Combine(folderPath, ".config", "VintagestoryData");
					if (Directory.Exists(text))
					{
						Directory.Move(text, Config);
					}
				}
			}
