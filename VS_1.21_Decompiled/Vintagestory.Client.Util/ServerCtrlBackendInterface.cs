...
public class ServerCtrlBackendInterface
{
	...
	public void GetGameVersions(OnActionComplete<string[]> onComplete)
	{
		try
		{
			JObject obj = JObject.Parse(webClient.GetStringAsync("http://api.vintagestory.at/stable-unstable.json").Result);
			List<string> versions = new List<string>();
			foreach (KeyValuePair<string, JToken> item2 in obj)
			{
				if (GameVersion.IsAtLeastVersion(item2.Key, "1.14.9"))
				{
					versions.Add(item2.Key);
				}
			}
	...
	public void GetVSHostingUnsupportedGameVersions(OnActionComplete<string[]> onComplete)
	{
		try
		{
			JObject obj = JObject.Parse(webClient.GetStringAsync("http://api.vintagestory.at/vshostingunsupported.json").Result);
			List<string> versions = new List<string>();
			foreach (JToken item in (IEnumerable<JToken>)obj["versions"])
			{
				versions.Add(((object)item).ToString());
			}
			ScreenManager.EnqueueMainThreadTask(delegate
			{
				onComplete(EnumAuthServerResponse.Good, versions.ToArray());
	...
	public void SetConfig(OnSrvActionComplete<GameServerStatus> onComplete, string serverconfig, string worldconfig)
	{
		...
		list.Add(new KeyValuePair<string, string>("serverconfig", serverconfig));
		list.Add(new KeyValuePair<string, string>("worldconfig", worldconfig));
		runAction(onComplete, "setconfig", list);
	}

	public void SelectRegion(string region, OnSrvActionComplete<ServerCtrlResponse> onComplete)
	{
		List<KeyValuePair<string, string>> list = new List<KeyValuePair<string, string>>();
		list.Add(new KeyValuePair<string, string>("region", region));
		runAction(onComplete, "selectregion", list);
	}

	public void SelectVersion(string version, OnSrvActionComplete<ServerCtrlResponse> onComplete)
	{
		List<KeyValuePair<string, string>> list = new List<KeyValuePair<string, string>>();
		list.Add(new KeyValuePair<string, string>("version", version));
		runAction(onComplete, "install", list);
	}

	public void DeleteMod(string mod, OnSrvActionComplete<ServerCtrlResponse> onComplete)
	{
		List<KeyValuePair<string, string>> list = new List<KeyValuePair<string, string>>();
		list.Add(new KeyValuePair<string, string>("mod", mod));
		runAction(onComplete, "deletemod", list);
	}
	...
	private void runAction<T>(OnSrvActionComplete<T> onComplete, string action, List<KeyValuePair<string, string>> postData = null) where T : ServerCtrlResponse
	{
		...
		FormUrlEncodedContent postData2 = new FormUrlEncodedContent(postData);
		...
		webClient.PostAsync(uri, postData2, delegate(CompletedArgs args)
		{
			ScreenManager.EnqueueMainThreadTask(delegate
			{
