...
public static class EntityPosExtensions
{
	public static void SetFromPacket(this EntityPos pos, Packet_EntityPosition packet, Entity entity)
	{
		...
		{
			entityAgent.BodyYawServer = CollectibleNet.DeserializeFloatPrecise(packet.BodyYaw);
		}
		((entity.SidedProperties == null) ? null : entity.GetInterface<IMountable>()?.ControllingControls)?.FromInt(packet.MountControls);
	}
}
