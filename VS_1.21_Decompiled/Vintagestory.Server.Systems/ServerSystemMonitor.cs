...
public class ServerSystemMonitor : ServerSystem
{
	...
	private void OnEvery60sec(float obj)
	{
		...
		if (accumTick % 15 == 0)
		{
			...
			if (!server.RecentClientLogins.IsEmpty)
			{
	...
	private void OnEntryAdded(EnumLogType logtype, string message, object[] args)
	{
		if (logtype == EnumLogType.Error || logtype == EnumLogType.Fatal)
		{
			...
			{
				string finalLogMessage = $"More then {server.Config.DieAboveErrorCount} errors detected. Shutting down now. Threshold can be changed in serverconfig.json \"DieAboveErrorCount\"";
				server.Stop("Too many errors detected. See server-main.log file", finalLogMessage, EnumLogType.Error);
			}
		}
	}
}
