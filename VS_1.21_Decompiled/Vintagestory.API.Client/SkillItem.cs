...
public class SkillItem : IDisposable
{
	...
	public SkillItem WithLetterIcon(ICoreClientAPI capi, string letter)
	{
		...
		Texture = capi.Gui.Icons.GenTexture(isize, isize, delegate(Context ctx, ImageSurface surface)
		{
			...
			double num = ((FontExtents)(ref fontExtents)).Ascent + GuiElement.scaled(2.0);
			capi.Gui.Text.DrawTextLine(ctx, cairoFont, letter, ((double)isize - ((TextExtents)(ref textExtents)).Width) / 2.0, ((double)isize - num) / 2.0);
		});
		return this;
	}
