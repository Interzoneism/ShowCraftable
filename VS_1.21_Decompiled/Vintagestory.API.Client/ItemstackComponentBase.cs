...
public class ItemstackComponentBase : RichTextComponentBase
{
	...
	public void RenderItemstackTooltip(ItemSlot slot, double renderX, double renderY, float dt)
	{
		...
		bool flag = (double)capi.Input.MouseX + stackInfoBounds.OuterWidth > (double)(capi.Render.FrameWidth - 5);
		bool flag2 = (double)capi.Input.MouseY + stackInfoBounds.OuterHeight > (double)(capi.Render.FrameHeight - 5);
		if (recalcAlignmentOffset || bottomOverlap != flag2 || flag != rightOverlap)
		{
			stackInfoBounds.WithFixedAlignmentOffset(flag ? ((0.0 - stackInfoBounds.OuterWidth) / (double)RuntimeEnv.GUIScale - (double)tooltipOffsetX) : 0.0, flag2 ? ((0.0 - stackInfoBounds.OuterHeight) / (double)RuntimeEnv.GUIScale - (double)tooltipOffsetY) : 0.0);
