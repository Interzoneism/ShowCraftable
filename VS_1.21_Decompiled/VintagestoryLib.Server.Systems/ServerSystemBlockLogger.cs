...
public class ServerSystemBlockLogger : ServerSystem
{
	...
	private void DidPlaceBLock(IServerPlayer byplayer, int oldblockid, BlockSelection blocksel, ItemStack withitemstack)
	{
		if (oldblockid != 0)
		{
			string text = sapi.World.GetBlock(oldblockid).Code.ToString();
			sapi.Logger.Build("{0} placed {1} [pre: {2}] at {3}", byplayer.PlayerName, withitemstack.Collectible.Code.ToString(), text, blocksel.Position);
		}
		else
		{
	...
	{
		string text = ((oldblockid != 0) ? sapi.World.GetBlock(oldblockid).Code.ToString() : "Air");
		sapi.Logger.Build("{0} removed {1} at {2}", byplayer.PlayerName, text, blocksel.Position);
	}
