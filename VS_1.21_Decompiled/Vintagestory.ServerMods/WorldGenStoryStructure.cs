...
public class WorldGenStoryStructure : WorldGenStructureBase
{
	...
	public void Init(ICoreServerAPI api, WorldGenStoryStructuresConfig scfg, RockStrataConfig rockstrata, BlockLayerConfig blockLayerConfig)
	{
		...
		scfg.SchematicYOffsets.TryGetValue("story/" + schematicData.FromFileName.Replace(".json", ""), out var value);
		schematicData.OffsetY = value;
		if (SkipGenerationCategories != null)
		{
			SkipGenerationFlags = new Dictionary<int, int>();
			foreach (KeyValuePair<string, int> skipGenerationCategory in SkipGenerationCategories)
			{
				SkipGenerationFlags.Add(BitConverter.ToInt32(SHA256.HashData(Encoding.UTF8.GetBytes(skipGenerationCategory.Key.ToLowerInvariant()))), skipGenerationCategory.Value);
			}
		}
