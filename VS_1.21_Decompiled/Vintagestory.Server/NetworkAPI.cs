...
public class NetworkAPI : IServerNetworkAPI, INetworkAPI
{
	...
	{
		if (channelsUdp.TryGetValue(packet.ChannelId, out var value))
		{
			value.OnPacket(packet, player);
		}
	}
	...
	public void SendChannelsPacket(IServerPlayer player)
	{
		...
		packet_NetworkChannels.SetChannelIds(channels.Keys.ToArray());
		packet_NetworkChannels.SetChannelNames(channels.Values.Select((NetworkChannel ch) => ch.ChannelName).ToArray());
		packet_NetworkChannels.SetChannelUdpIds(channelsUdp.Keys.ToArray());
		packet_NetworkChannels.SetChannelUdpNames(channelsUdp.Values.Select((UdpNetworkChannel ch) => ch.ChannelName).ToArray());
		server.SendPacket(player.ClientId, new Packet_Server
		{
			Id = 56,
			NetworkChannels = packet_NetworkChannels
		});
	}
	...
	{
		server.BroadcastArbitraryPacket(packet as Packet_Server, exceptPlayers);
	}
	...
	{
		server.SendArbitraryPacket(packet as Packet_Server, players);
	}
