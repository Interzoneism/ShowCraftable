...
internal class ServerSystemCalendar : ServerSystem
{
	...
	{
		ITreeAttribute worldConfiguration = server.SaveGameData.WorldConfiguration;
		int num = Math.Max(1, worldConfiguration.GetAsInt("daysPerMonth", 12));
		server.GameWorldCalendar = new GameCalendar(server.AssetManager.Get("textures/environment/sunlight.png"), server.SaveGameData.Seed, 28800 + 86400 * num * 4);
		server.GameWorldCalendar.DaysPerMonth = num;
	}
