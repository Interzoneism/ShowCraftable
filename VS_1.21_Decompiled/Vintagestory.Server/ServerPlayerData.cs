...
public class ServerPlayerData : IComparable<ServerPlayerData>, IServerPlayerData
{
	...
	public HashSet<string> GetAllPrivilegeCodes(ServerConfig serverConfig)
	{
		...
		hashSet.AddRange(PermaPrivileges);
		if (serverConfig.RolesByCode.TryGetValue(RoleCode, out var value))
		{
			hashSet.AddRange(value.Privileges);
			hashSet.AddRange(value.RuntimePrivileges);
		}
		foreach (string deniedPrivilege in DeniedPrivileges)
		{
			hashSet.Remove(deniedPrivilege);
		}
		hashSet.AddRange(RuntimePrivileges);
		return hashSet;
	}
	...
	public bool HasPrivilege(string privilege, Dictionary<string, PlayerRole> rolesByCode)
	{
		...
		if (value.Privileges.Contains(privilege))
		{
			return true;
		}
		if (value.RuntimePrivileges.Contains(privilege))
		{
			return true;
		}
		return false;
	...
	{
		server.Config.RolesByCode.TryGetValue(RoleCode, out var value);
		if (value == null)
		{
