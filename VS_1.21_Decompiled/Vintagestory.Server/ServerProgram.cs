...
public class ServerProgram
{
	...
	public ServerProgram()
	{
		...
		ParserResult<ServerProgramArgs> val = new Parser((Action<ParserSettings>)delegate(ParserSettings config)
		{
			config.HelpWriter = null;
			config.AutoHelp = false;
			config.AutoVersion = false;
		}).ParseArguments<ServerProgramArgs>((IEnumerable<string>)args);
		progArgs = val.Value;
		...
		{
			Console.WriteLine();
			Console.Write("1.21.0");
		}
		else if (progArgs.PrintHelp)
		{
			Console.WriteLine();
			Console.Write(progArgs.GetUsage(val));
		}
		else
		{
	...
	private void Main()
	{
		...
		ServerMain.Logger.Notification("CPU Cores: {0}", Environment.ProcessorCount);
		ServerMain.Logger.Notification("CPU: {0}", ServerMain.xPlatInterface.GetCpuInfo());
		ServerMain.Logger.Notification("Available RAM: {0} MB", ServerMain.xPlatInterface.GetRamCapacity() / 1024);
