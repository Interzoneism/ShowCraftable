...
internal class CmdServerConfig
{
	...
	public CmdServerConfig(ServerMain server)
	{
		...
		chatCommands.Create("serverconfig").WithAlias("sc").WithDesc("Read or Set server configuration")
			...
			.WithArgs(parsers.OptionalAll("all"))
			.HandleWith((TextCommandCallingArgs _) => TextCommandResult.Success("Please use /worldconfig allowFireSpread true/false to change the value"))
			.EndSub()
			.BeginSub("allowfallingblocks")
			.WithAlias("fallingblocks")
			.WithDesc("Whether to allow falling block physics")
			.WithArgs(parsers.OptionalAll("all"))
			.HandleWith((TextCommandCallingArgs _) => TextCommandResult.Success("Please use /worldconfig allowFallingBlocks true/false to change the value"))
			.EndSub()
			...
			.HandleWith(delegate(TextCommandCallingArgs args)
			{
				...
				server.Config.TickTime = 1000f / num;
				server.ConfigNeedsSaving = true;
				return TextCommandResult.Success(Lang.Get("Ok, tick rate now at {0} tp/s", num), EnumChatType.CommandSuccess);
			})
			.EndSub()
			...
			.HandleWith(delegate(TextCommandCallingArgs args)
			{
				...
				int num = GameMath.Max((int)args[0], 30);
				MagicNum.ServerAutoSave = num;
				MagicNum.Save();
				return TextCommandResult.Success(Lang.Get("Ok, autosave interval now at {0} s", num));
			})
			.EndSub()
	...
	private TextCommandResult getOrSet(string name, TextCommandCallingArgs args)
	{
		...
		if (args.Parsers[0].IsMissing)
		{
			object obj = ((!(name == "EntitySpawning")) ? server.Config.Get(name) : ((object)server.SaveGameData.EntitySpawning));
			if (obj is bool)
			{
				obj = (((bool)obj) ? "on" : "off");
			}
			return TextCommandResult.Success(Lang.Get("Current {0} is {1}", args.Parsers[0].ArgumentName, obj));
		}
		object obj2 = args[0];
		if (obj2 is bool)
		{
			obj2 = (((bool)obj2) ? "on" : "off");
		}
		...
		ConfigNeedsSaving = true;
		ServerMain.Logger.Audit(Lang.Get("{0} changes server config {1} to {2}.", args.Caller.GetName(), name, obj2));
		return TextCommandResult.Success(Lang.Get("{0} set to {1}", args.Parsers[0].ArgumentName, obj2));
	}
	...
	private TextCommandResult handleSetSpawn(TextCommandCallingArgs args)
	{
		...
		{
			num2 = (int)args[1];
			num3 = server.WorldMap.GetTerrainGenSurfacePosY(num, num2);
		}
		else
		{
			num3 = (int)args[1];
		...
		if (!server.WorldMap.IsValidPos(num, num3, num2))
		{
		...
		ConfigNeedsSaving = true;
		return TextCommandResult.Success(Lang.Get("Default spawnpoint now set to ={0} ={1} ={2}", num, num3, num2));
	}

	private TextCommandResult handleSetSpawnhere(TextCommandCallingArgs args)
	{
		BlockPos asBlockPos = args.Caller.Pos.AsBlockPos;
		if (!server.WorldMap.IsValidPos(asBlockPos.X, asBlockPos.Y, asBlockPos.Z))
		{
