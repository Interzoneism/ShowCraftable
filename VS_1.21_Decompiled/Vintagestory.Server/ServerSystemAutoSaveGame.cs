...
public class ServerSystemAutoSaveGame : ServerSystem
{
	...
	{
		if (!server.readyToAutoSave)
		{
			return TextCommandResult.Success("Server not ready to autosave now, try again.");
		}
		if (server.chunkThread.BackupInProgress)
		{
			return TextCommandResult.Success("Server is currently doing a backup. Will ignore autosave.");
		}
		doAutoSave();
