...
public class ServerSystemHeartbeat : ServerSystem
{
	...
	private void SendRegister()
	{
		...
		bool whitelisted = server.Config.WhitelistMode == EnumWhitelistMode.On || (server.Config.WhitelistMode == EnumWhitelistMode.Default && server.IsDedicatedServer);
		RegisterRequestPacket packet = new RegisterRequestPacket
		{
			gameVersion = "1.21.0",
			maxPlayers = (ushort)server.Config.MaxClients,
	...
	private async void SendRequestAsync<T>(string url, T packet, Action<ResponsePacket> onComplete)
	{
		...
		try
		{
			json = JsonConvert.SerializeObject((object)packet);
			StringContent content = new StringContent(json, null, "application/json");
			HttpResponseMessage httpResponseMessage = await VSWebClient.Inst.PostAsync(url, content);
		...
		{
			ServerMain.Logger.Warning("Socket exception on master server async request: {0}", ex.Message);
			onComplete(new ResponsePacket
			{
				data = null,
				status = "timeout"
