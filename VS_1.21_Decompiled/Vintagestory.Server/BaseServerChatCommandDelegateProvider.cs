...
public abstract class BaseServerChatCommandDelegateProvider : LegacyServerChatCommand
{
	...
	protected bool CanKickOrBanTarget(int groupId, IServerPlayer issuingPlayer, string targetPlayerName)
	{
		...
		PlayerRole clientGroup = Config.RolesByCode[GetClient(issuingPlayer).ServerData.RoleCode];
		if (playerRole.IsSuperior(clientGroup) || (playerRole.EqualLevel(clientGroup) && !issuingPlayer.HasPrivilege(Privilege.root)))
		{
