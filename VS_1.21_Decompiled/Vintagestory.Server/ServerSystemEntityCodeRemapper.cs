...
public class ServerSystemEntityCodeRemapper : ServerSystem
{
	...
	private TextCommandResult OnCmdReMap(TextCommandCallingArgs args)
	{
		...
		string newCode = args[0] as string;
		string oldCode = args[1] as string;
		bool force = !args.Parsers[2].IsMissing && args[2] as string == "force";
		IServerPlayer player = args.Caller.Player as IServerPlayer;
		Addmapping(server.EntityCodeRemappings, newCode, oldCode, player, args.Caller.FromChatGroupId, remap: true, force, quiet);
	...
	private TextCommandResult OnCmdMap(TextCommandCallingArgs args)
	{
		...
		bool force = !args.Parsers[2].IsMissing && args[2] as string == "force";
		IServerPlayer player = args.Caller.Player as IServerPlayer;
		Addmapping(server.EntityCodeRemappings, newCode, oldCode, player, args.Caller.FromChatGroupId, remap: false, force, quiet: false);
		return TextCommandResult.Success();
	}
	...
	private TextCommandResult OnCmdList(TextCommandCallingArgs args)
	{
		...
		foreach (KeyValuePair<string, string> item in entityCodeRemappings)
		{
	...
	{
		if (!force && entityRemaps.TryGetValue(oldCode, out var value))
		{
			player.SendMessage(groupId, "new entity code " + oldCode + " is already mapped to " + value + ", type '/eir " + (remap ? "remap" : "map") + " " + newCode + " " + oldCode + " force' to overwrite", EnumChatType.CommandError);
		}
		else
		{
			...
			if (!quiet)
			{
				...
				player.SendMessage(groupId, newCode + " is now " + text + " from entity code " + oldCode, EnumChatType.CommandSuccess);
			}
		}
	}
}
