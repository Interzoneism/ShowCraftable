...
public class ConnectedClient
{
	...
	public BlockPos ChunkPos
	{
		get
		{
			...
			return new BlockPos((int)position.X / 32, (int)position.Y / 32, (int)position.Z / 32, position.Dimension);
		}
	}
	...
	public void Initialise()
	{
		...
		TrackedEntities = new HashSet<long>(100);
		EntitySpawnsToSend = new List<Entity>();
	}
	...
	public void LoadOrCreatePlayerData(ServerMain server, string playername, string playerUid)
	{
		...
		{
			byte[] playerData = server.chunkThread.gameDatabase.GetPlayerData(playerUid);
			if (playerData != null)
			{
				try
				{
				...
				catch (Exception ex)
				{
					...
					ServerMain.Logger.Notification(LoggerBase.CleanStackTrace(ex.ToString()));
				}
			}
			if (playerData == null)
			{
				...
				worlddata.Init(server);
				EntityProperties entityType = server.GetEntityType(GlobalConstants.EntityPlayerTypeCode);
				if (entityType == null)
				{
				...
				worlddata.EntityPlayer.Code = entityType.Code;
		...
		if (worlddata.EntityPlayer == null)
		{
			...
			EntityProperties entityType2 = server.GetEntityType(GlobalConstants.EntityPlayerTypeCode);
			if (entityType2 == null)
			{
				throw new Exception(string.Concat("Cannot init player, there is no entity type with code ", GlobalConstants.EntityPlayerTypeCode, " was loaded!"));
			}
			worlddata.EntityPlayer.Code = entityType2.Code;
			IsNewEntityPlayer = true;
		}
	...
	{
		string text = Entityplayer.WatchedAttributes.GetString("name");
		return $"{text}:{ServerData.RoleCode}:{PlayerRole.PrivilegesString(ServerData.PermaPrivileges.ToList())} {ipAddress}";
	}
