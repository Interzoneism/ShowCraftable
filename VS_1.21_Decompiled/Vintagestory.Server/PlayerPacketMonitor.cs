...
public class PlayerPacketMonitor : ServerSystem
{
	...
	{
		foreach (KeyValuePair<int, MonitorClient> monitorClient in monitorClients)
		{
			monitorClient.Value.BlocksSet = 0;
			monitorClient.Value.MessagesSent = 0;
			monitorClient.Value.PacketsReceived = 0;
			monitorClient.Value.PacketsReceivedById = new int[100];
		}
	}
	...
	public bool CheckPacket(int clientId, Packet_Client packet)
	{
		...
		ConnectedClient connectedClient = server.Clients[clientId];
		if (HaveOverflow(monitorClient, packet.Id))
		{
			_ = connectedClient.PlayerName;
			string othersKickmessage = Lang.Get("Automatically kicked by packet monitor, reason: {0}", "Packet overflow");
			server.DisconnectPlayer(connectedClient, othersKickmessage);
			int mostsendpacketId = monitorClient.PacketsReceivedById.ToList().IndexOf(monitorClient.PacketsReceivedById.Max());
			ServerMain.Logger.Notification(OverflowReason(monitorClient, packet.Id, mostsendpacketId));
			return false;
		}
		switch (packet.Id)
		{
		...
		case 4:
			...
			{
				server.SendMessage(connectedClient.Player, packet.Chatline.Groupid, Lang.Get("Spam protection in place, message not sent"), EnumChatType.Notification);
				return false;
			}
			...
			return ActionMessage(connectedClient.Player, packet.Chatline.Groupid);
		default:
			return true;
		}
	}
	...
	private bool ActionMessage(IServerPlayer player, int groupid)
	{
		...
		string message = Lang.Get("You've sent too many message at once, you've been muted for {0} seconds", config.MessageBanTime);
		player.SendMessage(groupid, message, EnumChatType.Notification);
		return false;
	}
	...
	public void SaveConfig()
	{
		...
		textWriter.Write(JsonConvert.SerializeObject((object)config));
		textWriter.Close();
	}
}
