...
public class ParticleRenderer2D
{
	...
	public void Compose(string texture)
	{
		if (texture != null)
		{
			BitmapRef bitmapRef = screenManager.GamePlatform.AssetManager.Get(texture).ToBitmap(screenManager.api);
			particleTex = ScreenManager.Platform.LoadTexture((IBitmap)bitmapRef, linearMag: false, 0, generateMipmaps: false);
			bitmapRef.Dispose();
		}
	}
