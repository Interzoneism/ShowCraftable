...
public class SystemHotkeys : ClientSystem
{
	...
	{
		string key = comp.Href.Substring("hotkey://".Length);
		if (ScreenManager.hotkeyManager.HotKeys.TryGetValue(key, out var value))
		{
			value.Handler(value.CurrentMapping);
		}
	}
	...
	{
		float num = (viaKeyComb.Shift ? 0.1f : 1f);
		float num2 = (float)Math.Max(num, Math.Round(10f * (game.player.worlddata.MoveSpeedMultiplier - num)) / 10.0);
		game.player.worlddata.SetMode(game, num2);
		game.ShowChatMessage($"Movespeed: {num2}");
		return true;
	}
	...
	{
		ItemSlot itemSlot = game.player.inventoryMgr.currentHoveredSlot;
		if (itemSlot == null)
		{
			itemSlot = game.player.inventoryMgr.ActiveHotbarSlot;
		}
		if (game.player.inventoryMgr.DropItem(itemSlot, fullStack: false))
		{
	...
	private bool KeyDropItems(KeyCombination viaKeyComb)
	{
		...
		if (game.player.inventoryMgr.DropItem(itemSlot, fullStack: true))
		{
			game.PlaySound(new AssetLocation("sounds/player/quickthrow"), randomizePitch: true);
		}
		return true;
	...
	private bool KeyReloadShaders(KeyCombination viaKeyComb)
	{
		...
		flag = flag && flag2;
		game.ShowChatMessage("Shaders reloaded" + (flag ? "" : ". errors occured, please check client log"));
		return true;
	}
