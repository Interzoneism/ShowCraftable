...
internal class GuiScreenGetUpdate : GuiScreen
{
	...
	private void Compose()
	{
		...
		ElementBounds elementBounds = ElementBounds.Fixed(0.0, 0.0, 500.0, 50.0);
		ElementBounds elementBounds2 = ElementBounds.Fixed(0.0, 90.0, 0.0, 0.0).WithAlignment(EnumDialogArea.CenterFixed).WithFixedPadding(10.0, 2.0);
		ElementComposer = dialogBase("mainmenu-confirmaction", -1.0, 160.0).AddStaticText(Lang.Get("Download in progress"), CairoFont.WhiteSmallishText().WithWeight((FontWeight)1), elementBounds).AddDynamicText("Downloading releases meta information...", CairoFont.WhiteSmallText(), elementBounds = elementBounds.BelowCopy(0.0, 10.0).WithFixedWidth(500.0), "status").AddButton(Lang.Get("Cancel"), OnCancel, elementBounds2.FixedUnder(elementBounds, 10.0))
			.EndChildElements()
			.Compose();
	}
	...
	private void BeginDownload()
	{
		Task.Run(async delegate
		{
			...
			{
				onReleasesDownloadComplete(errorExc);
			}
		});
	}
