...
public class BEBehaviorMPAngledGears : BEBehaviorMPBase
{
	...
	public override float AngleRad
	{
		get
		{
			float angleRad = base.AngleRad;
			bool flag = propagationDir == BlockFacing.DOWN || propagationDir == BlockFacing.WEST;
			...
			return (float)Math.PI * 2f - angleRad;
		}
	}
	...
	private void onEverySecond(float dt)
	{
		...
		if (Api.World.Rand.NextDouble() < (double)(num / 3f))
		{
			Api.World.PlaySoundAt(new AssetLocation("sounds/block/woodcreak"), (double)Position.X + 0.5, (double)Position.Y + 0.5, (double)Position.Z + 0.5, null, 0.75f + num);
		}
	}

	public override void SetOrientations()
	{
		string text = (base.Block as BlockAngledGears).Orientation;
		if (turnDir1 != null)
		{
	...
	protected void CheckLargeGearJoin()
	{
		...
		{
			BlockPos position = Position.AddCopy(orientation.Opposite);
			BlockEntity blockEntity = Api?.World.BlockAccessor.GetBlockEntity(position);
			if (blockEntity != null)
			{
				SetLargeGear(blockEntity);
			}
		}
	}
	...
	public override void SetPropagationDirection(MechPowerPath path)
	{
		...
		if (turnDir1 != null)
		{
			if (blockFacing == turnDir1)
			{
				blockFacing = turnDir2.Opposite;
			}
			else if (blockFacing == turnDir2)
			{
				blockFacing = turnDir1.Opposite;
			}
			else if (blockFacing == turnDir2.Opposite)
			{
				blockFacing = turnDir1;
			}
			else if (blockFacing == turnDir1.Opposite)
			{
				blockFacing = turnDir2;
			}
		}
		path = new MechPowerPath(blockFacing, path.gearingRatio);
	...
	protected override MechPowerPath[] GetMechPowerExits(MechPowerPath fromExitTurnDir)
	{
		...
		if (text.Length < 2 || text[0] != text[1])
		{
			bool flag = fromExitTurnDir.invert;
			BlockFacing[] array = (base.Block as BlockAngledGears).Facings;
			BlockFacing blockFacing = fromExitTurnDir.OutFacing;
			if (!array.Contains(blockFacing))
			{
			...
			if (!newlyPlaced)
			{
				array = array.Remove(blockFacing);
			}
			MechPowerPath[] array2 = new MechPowerPath[array.Length];
			for (int i = 0; i < array2.Length; i++)
			{
				BlockFacing blockFacing2 = array[i];
				array2[i] = new MechPowerPath(blockFacing2, base.GearedRatio, null, (blockFacing2 == blockFacing) ? flag : (!flag));
			}
			return array2;
		}
	...
	public override void GetBlockInfo(IPlayer forPlayer, StringBuilder sb)
	{
		...
		if (Api.World.EntityDebugMode)
		{
			...
			bool flag = propagationDir == axis1 || propagationDir == axis2;
			sb.AppendLine(string.Format(Lang.Get("Orientation: {0} {1} {2}", text, orientation, flag ? "-" : "")));
		}
	}
