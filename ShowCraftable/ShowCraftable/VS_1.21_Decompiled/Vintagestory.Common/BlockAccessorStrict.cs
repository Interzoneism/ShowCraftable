...
internal class BlockAccessorStrict : BlockAccessorBase
{
	...
	public override int GetBlockId(int posX, int posY, int posZ, int layer)
	{
		...
		if (chunkAtPos != null)
		{
			return chunkAtPos.UnpackAndReadBlock(worldmap.ChunkSizedIndex3D(posX & 0x1F, posY & 0x1F, posZ & 0x1F), layer);
		}
	...
	public override void ExchangeBlock(int blockId, BlockPos pos)
	{
		...
		IWorldChunk chunkAtPos = worldmap.GetChunkAtPos(pos.X, pos.InternalY, pos.Z);
		if (chunkAtPos != null)
		{
			chunkAtPos.Unpack();
			int index3d = worldmap.ChunkSizedIndex3D(pos.X & 0x1F, pos.InternalY & 0x1F, pos.Z & 0x1F);
			int oldblockid = chunkAtPos.Data[index3d];
			chunkAtPos.Data[index3d] = blockId;
			chunkAtPos.MarkModified();
			Block block = worldmap.Blocks[blockId];
			if (!block.ForFluidsLayer)
			{
				chunkAtPos.GetLocalBlockEntityAtBlockPos(pos)?.OnExchanged(block);
			}
			if (synchronize)
			{
