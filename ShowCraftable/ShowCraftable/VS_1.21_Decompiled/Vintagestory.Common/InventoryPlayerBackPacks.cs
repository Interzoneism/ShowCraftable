...
public class InventoryPlayerBackPacks : InventoryBasePlayer
{
	...
	{
		if (this[slotId] is ItemSlotBagContent itemSlotBagContent)
		{
			base.PerformNotifySlot(itemSlotBagContent.BagIndex);
		}
	...
	{
		int despawnSeconds = (base.Player?.Entity?.Properties.Attributes)?["droppedItemsOnDeathTimer"].AsInt(GlobalConstants.TimeToDespawnPlayerInventoryDrops) ?? GlobalConstants.TimeToDespawnPlayerInventoryDrops;
		for (int i = 0; i < bagSlots.Length; i++)
		{
			ItemSlot itemSlot = bagSlots[i];
