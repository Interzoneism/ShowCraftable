...
public class ParticleManager
{
	...
	private void copyLoadedEntitiesList2sec(float dt)
	{
		...
		foreach (KeyValuePair<long, Entity> loadedEntity in particleSystem.game.LoadedEntities)
		{
			dictionary[loadedEntity.Key] = loadedEntity.Value;
		}
		loadedEntities = dictionary;
	}
	...
	private bool AsyncParticleSpawnTick(float dt, IAsyncParticleManager manager)
	{
		...
		lock (asyncParticleQueueLock)
		{
			...
			{
				IParticlePropertiesProvider particlePropertiesProvider = asyncParticleQueue.Dequeue();
				particleSystem.offthreadpools[(int)particlePropertiesProvider.ParticleModel].SpawnParticles(particlePropertiesProvider);
			}
		}
		return true;
	}
	...
	public void SpawnParticles(float quantity, int color, Vec3d minPos, Vec3d maxPos, Vec3f minVelocity, Vec3f maxVelocity, float lifeLength, float gravityEffect, float scale, EnumParticleModel model)
	{
		...
		simpleParticleProperties.Init(particleSystem.game.api);
		simpleParticleProperties.ParticleModel = model;
		simpleParticleProperties.MinSize = (simpleParticleProperties.MaxSize = scale);
		particleSystem.mainthreadpools[(int)simpleParticleProperties.ParticleModel].SpawnParticles(simpleParticleProperties);
	}
	...
	{
		bool flag = Environment.CurrentManagedThreadId == RuntimeEnv.MainThreadId;
		if (flag && properties.Async)
		{
