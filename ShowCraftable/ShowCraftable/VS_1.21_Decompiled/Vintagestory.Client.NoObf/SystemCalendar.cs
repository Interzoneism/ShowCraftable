...
public class SystemCalendar : ClientSystem
{
	...
	private void HandleCalendarPacket(Packet_Server packet)
	{
		if (!game.ignoreServerCalendarUpdates && game.GameWorldCalendar != null)
		{
			...
			int num = (int)(packet.Calendar.TotalSeconds - game.GameWorldCalendar.TotalSeconds);
			if (Math.Abs(num) > 900 && game.GameWorldCalendar.TotalSeconds > 28000)
			{
				ScreenManager.Platform.Logger.Notification("Wow, client daytime drifted off significantly from server daytime ({0} mins)", Math.Round((float)num / 60f, 1));
			}
	...
	{
		GameCalendar gameWorldCalendar = game.GameWorldCalendar;
		game.ShowChatMessage("Client time: " + gameWorldCalendar.PrettyDate());
		if ((bool)args[0])
		{
