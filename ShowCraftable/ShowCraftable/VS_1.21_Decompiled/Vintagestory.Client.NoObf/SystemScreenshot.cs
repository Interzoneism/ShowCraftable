...
public class SystemScreenshot : ClientSystem
{
	...
	private void AfterFinalCompo(float dt)
	{
		...
		{
			string path = Path.Combine(GamePaths.Screenshots, "timelapse");
	...
	private void DoScreenshot(string path)
	{
		...
		try
		{
			string text = game.Platform.SaveScreenshot(path, null, withAlpha: false, ClientSettings.FlipScreenshot, GetMetaData());
			if (path == null)
			{
				string message = ((doTakeMegaScreenshot > 0) ? Lang.Get("screenshottaken-mega", text) : Lang.Get("screenshottaken-normal", text));
	...
	public void OnFrameDone(float dt)
	{
		if (!usePrimaryFramebuffer && takeScreenshot && game.Platform.EllapsedMs - nextScreenshotdelay > 1000)
		{
			...
			try
			{
				string text = game.Platform.SaveScreenshot(null, null, withAlpha: false, ClientSettings.FlipScreenshot, GetMetaData());
				string message = ((doTakeMegaScreenshot > 0) ? Lang.Get("screenshottaken-mega", text) : Lang.Get("screenshottaken-normal", text));
				game.ShowChatMessage(message);
			}
			catch (Exception e)
