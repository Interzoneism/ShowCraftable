...
public class ClientPlayer : IClientPlayer, IPlayer
{
	...
	{
		string inventoryId = packet.InventoryId;
		ScreenManager.Platform.Logger.VerboseDebug("Received inventory contents " + inventoryId);
		if (packet.InventoryClass == null)
		{
			throw new Exception("Illegal inventory contents packet, classname is null! " + packet.InventoryId);
		}
		if (!inventoryMgr.Inventories.ContainsKey(inventoryId))
		{
			inventoryMgr.Inventories[inventoryId] = (InventoryBasePlayer)ClientMain.ClassRegistry.CreateInventory(packet.InventoryClass, packet.InventoryId, game.api);
		}
		(inventoryMgr.Inventories[inventoryId].InvNetworkUtil as InventoryNetworkUtil).UpdateFromPacket(game, packet);
	}
	...
	public void UpdateFromPacket(ClientMain game, Packet_PlayerData packet)
	{
		...
		for (int j = 0; j < packet.InventoryContentsCount; j++)
		{
			AddOrUpdateInventory(game, packet.InventoryContents[j]);
		}
	...
	public PlayerGroupMembership GetGroup(int groupId)
	{
		...
		game.OwnPlayerGroupMemembershipsById.TryGetValue(groupId, out var value);
		return value;
	}
	...
	internal void WarnIfEntityChanged(long newId, string packetName)
	{
		...
		Entity value;
		bool flag = game.LoadedEntities.TryGetValue(entityId, out value);
		game.Logger.Warning("Old entityID {0} loaded: {1}.  New entityID {2} loaded: {3}.", entityId, flag, newId, game.LoadedEntities.ContainsKey(newId));
		if (flag && value != null)
		{
			game.Logger.Warning("Attempting to despawn the old entityID");
			try
			{
				EntityDespawnData despawn = new EntityDespawnData
				{
				...
				value.OnEntityDespawn(despawn);
				game.RemoveEntityRenderer(value);
				value.Properties.Client.Renderer = null;
				game.WorldMap.GetClientChunk(value.InChunkIndex3d)?.RemoveEntity(entityId);
