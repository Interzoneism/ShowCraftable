...
public class BlockCactus : BlockPlant
{
	...
	public override bool TryPlaceBlockForWorldGen(IBlockAccessor blockAccessor, BlockPos pos, BlockFacing onBlockFace, IRandom worldGenRand, BlockPatchAttributes attributes = null)
	{
		...
		int num = rand.Next(3);
		Block[] array = blocksByHeight[num];
		if (array.Length == 3)
		{
			if (rand.Next(6) == 0)
			{
			...
			{
				array[2] = topFlowering;
			}
			else
			{
				array[2] = blocksByHeight[0][0];
			}
		}
		for (int i = 0; i < array.Length && blockAccessor.GetBlock(pos).IsReplacableBy(array[i]); i++)
		{
			blockAccessor.SetBlock(array[i].BlockId, pos);
			pos = pos.Up();
		}
		return true;
	}
