...
public class BlockBed : Block, IClaimTraverseable
{
	...
	{
		BlockPos blockPos = new BlockPos(tree.GetInt("posx"), tree.GetInt("posy"), tree.GetInt("posz"));
		Block block = world.BlockAccessor.GetBlock(blockPos);
		BlockFacing facing = BlockFacing.FromCode(block.LastCodePart());
		return world.BlockAccessor.GetBlockEntity((block.LastCodePart(1) == "feet") ? blockPos.AddCopy(facing) : blockPos) as BlockEntityBed;
	}
	...
	public override bool TryPlaceBlock(IWorldAccessor world, IPlayer byPlayer, ItemStack itemstack, BlockSelection blockSel, ref string failureCode)
	{
		...
		if (CanPlaceBlock(world, byPlayer, blockSel, ref failureCode))
		{
			...
			if (!CanPlaceBlock(world, byPlayer, blockSel2, ref failureCode))
			{
				return false;
			}
			string code = array[0].Opposite.Code;
			world.BlockAccessor.GetBlock(CodeWithParts("head", code)).DoPlaceBlock(world, byPlayer, blockSel2, itemstack);
			AssetLocation code2 = CodeWithParts("feet", code);
	...
	public override bool OnBlockInteractStart(IWorldAccessor world, IPlayer byPlayer, BlockSelection blockSel)
	{
		...
		if (!(world.BlockAccessor.GetBlockEntity((LastCodePart(1) == "feet") ? blockSel.Position.AddCopy(opposite) : blockSel.Position) is BlockEntityBed blockEntityBed))
		{
			return false;
		}
		if (blockEntityBed.MountedBy != null)
		{
			return false;
		}
		...
		return byPlayer.Entity.TryMount(blockEntityBed);
	}
	...
	public override void OnBlockRemoved(IWorldAccessor world, BlockPos pos)
	{
		...
		if (block is BlockBed && block.LastCodePart(1) != text)
		{
	...
	public override void GetHeldItemInfo(ItemSlot inSlot, StringBuilder dsc, IWorldAccessor world, bool withDebugInfo)
	{
		...
		double num = inSlot.Itemstack.Collectible.Attributes["sleepEfficiency"].AsDouble() * (double)world.Calendar.HoursPerDay / 2.0;
		dsc.AppendLine("\n" + Lang.Get("Lets you sleep for {0} hours a day", num.ToString("#.#")));
	}
	...
	public override string GetPlacedBlockInfo(IWorldAccessor world, BlockPos pos, IPlayer forPlayer)
	{
		...
		{
			num = Attributes["sleepEfficiency"].AsFloat(0.5f);
		}
		return base.GetPlacedBlockInfo(world, pos, forPlayer) + Lang.Get("Lets you sleep for up to {0} hours", Math.Round(num * world.Calendar.HoursPerDay / 2f, 2));
	}
