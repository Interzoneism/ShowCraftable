...
public class BlockEntityPeatPile : BlockEntityItemPile, ITexPositionSource
{
	...
	private MeshData[] GenMeshes()
	{
		...
		Shape shapeBase = Shape.TryGet(Api, "shapes/block/peatpile.json");
		ITesselatorAPI tesselator = ((ICoreClientAPI)Api).Tesselator;
		MeshData[] array = new MeshData[33];
		for (int i = 0; i <= 32; i++)
		{
			tesselator.TesselateShape("peatPile", shapeBase, out array[i], this, null, 0, 0, 0, i);
		}
