...
public class BlockIngotPile : Block
{
	...
	public int FillLevel(IBlockAccessor blockAccessor, BlockPos pos)
	{
		...
		if (blockEntity is BlockEntityIngotPile)
		{
			return (int)Math.Ceiling((double)((BlockEntityIngotPile)blockEntity).OwnStackSize / 8.0);
		}
		return 1;
	}
	...
	public override bool OnBlockInteractStart(IWorldAccessor world, IPlayer byPlayer, BlockSelection blockSel)
	{
		...
		if (blockEntity is BlockEntityIngotPile)
		{
			return ((BlockEntityIngotPile)blockEntity).OnPlayerInteract(byPlayer);
		}
		return false;
	}
	...
	internal bool Construct(ItemSlot slot, IWorldAccessor world, BlockPos pos, IPlayer player)
	{
		...
		if (blockEntity is BlockEntityIngotPile)
		{
			BlockEntityIngotPile blockEntityIngotPile = (BlockEntityIngotPile)blockEntity;
			if (player.WorldData.CurrentGameMode == EnumGameMode.Creative)
			{
				blockEntityIngotPile.inventory[0].Itemstack = slot.Itemstack.Clone();
				blockEntityIngotPile.inventory[0].Itemstack.StackSize = 1;
			}
			else
			{
				blockEntityIngotPile.inventory[0].Itemstack = slot.TakeOut(player.Entity.Controls.CtrlKey ? blockEntityIngotPile.BulkTakeQuantity : blockEntityIngotPile.DefaultTakeQuantity);
			}
			blockEntityIngotPile.MarkDirty();
			world.BlockAccessor.MarkBlockDirty(pos);
			world.PlaySoundAt(new AssetLocation("sounds/block/ingot"), pos, (double)(blockEntityIngotPile.inventory[0].Itemstack.StackSize / blockEntityIngotPile.MaxStackSize) - 0.5, player, randomizePitch: false);
		}
	...
	{
		if (!(capi.World.BlockAccessor.GetBlockEntity(pos) is BlockEntityIngotPile blockEntityIngotPile))
		{
			return base.GetRandomColor(capi, pos, facing, rndIndex);
		}
		if (blockEntityIngotPile.MetalType == null)
		{
			return base.GetRandomColor(capi, pos, facing, rndIndex);
		}
		return capi.BlockTextureAtlas.GetRandomColor(Textures[blockEntityIngotPile.MetalType].Baked.TextureSubId, rndIndex);
	}
	...
	public override ItemStack OnPickBlock(IWorldAccessor world, BlockPos pos)
	{
		...
		if (blockEntity is BlockEntityIngotPile)
		{
			ItemStack itemstack = ((BlockEntityIngotPile)blockEntity).inventory[0].Itemstack;
			if (itemstack != null)
			{
			...
			new WorldInteraction
			{
				...
				GetMatchingStacks = delegate(WorldInteraction wi, BlockSelection bs, EntitySelection es)
				{
					if (world.BlockAccessor.GetBlockEntity(bs.Position) is BlockEntityIngotPile blockEntityIngotPile && blockEntityIngotPile.MaxStackSize > blockEntityIngotPile.inventory[0].StackSize && blockEntityIngotPile.inventory[0].Itemstack != null)
					{
						...
						itemStack.StackSize = blockEntityIngotPile.DefaultTakeQuantity;
