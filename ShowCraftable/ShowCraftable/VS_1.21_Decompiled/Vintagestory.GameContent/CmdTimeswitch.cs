...
public class CmdTimeswitch
{
	...
	public CmdTimeswitch(ICoreServerAPI api)
	{
		...
		chatCommands.Create("timeswitch").WithDescription("Timeswitch and dimensions switching commands").RequiresPrivilege(Privilege.controlserver)
	...
	private TextCommandResult ToggleState(TextCommandCallingArgs args)
	{
		...
		sapi.ModLoader.GetModSystem<Timeswitch>().ActivateTimeswitchServer(player, raiseToWorldSurface: false, out var _);
	...
	private TextCommandResult SetPos(TextCommandCallingArgs args)
	{
		...
		sapi.ModLoader.GetModSystem<Timeswitch>().SetPos(asBlockPos);
		return TextCommandResult.Success();
	}
	...
	private TextCommandResult Start(TextCommandCallingArgs args)
	{
		...
		modSystem.OnStartCommand(player);
		return TextCommandResult.Success();
	}
}
