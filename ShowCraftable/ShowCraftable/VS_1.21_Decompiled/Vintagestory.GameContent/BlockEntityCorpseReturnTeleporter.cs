...
public class BlockEntityCorpseReturnTeleporter : BlockEntityTeleporterBase
{
	...
	{
		if (forEntity is EntityPlayer entityPlayer)
		{
			IServerPlayer serverPlayer = entityPlayer.Player as IServerPlayer;
			if (Api.ModLoader.GetModSystem<ModSystemCorpseReturnTeleporter>().lastDeathLocations.TryGetValue(serverPlayer.PlayerUID, out var value))
			{
				return value;
			}
		}
	...
	public override void Initialize(ICoreAPI api)
	{
		...
		if (api.World.Side == EnumAppSide.Client)
		{
			...
			animUtil.InitializeAnimator("corpsereturnteleporter", null, null, new Vec3f(0f, rotateY, 0f));
	...
	public bool OnInteract(IPlayer byPlayer)
	{
		...
		if (!HasFuel && !activeHotbarSlot.Empty)
		{
