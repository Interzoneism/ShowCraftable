...
public class ModSleeping : ModSystem
{
	...
	private void FastTick(float dt)
	{
		if (api.Side == EnumAppSide.Client)
		{
			...
			bool flag = capi.World?.Player?.Entity?.MountedOn is BlockEntityBed;
			sleepLevel = GameMath.Clamp(sleepLevel + dt * ((flag && AllSleeping) ? 0.1f : (-0.35f)), 0f, 0.99f);
		}
	...
	public void WakeAllPlayers()
	{
		...
		foreach (IPlayer player in allOnlinePlayers)
		{
			...
			if (serverPlayer.ConnectionState == EnumClientState.Playing && serverPlayer.WorldData.CurrentGameMode != EnumGameMode.Spectator)
			{
				EntityBehaviorTiredness? behavior2 = serverPlayer.Entity.GetBehavior<EntityBehaviorTiredness>();
				IMountableSeat mountableSeat = player.Entity?.MountedOn;
				if (behavior2 != null && behavior2.IsSleeping && mountableSeat != null)
				{
	...
	{
		bool flag = AreAllPlayersSleeping();
		if (flag != AllSleeping)
		{
			if (flag)
			{
	...
	public bool AreAllPlayersSleeping()
	{
		...
		for (int i = 0; i < allOnlinePlayers.Length; i++)
		{
			...
			if (serverPlayer.ConnectionState == EnumClientState.Playing && serverPlayer.WorldData.CurrentGameMode != EnumGameMode.Spectator)
			{
				EntityBehaviorTiredness? behavior = serverPlayer.Entity.GetBehavior<EntityBehaviorTiredness>();
				if (behavior != null && behavior.IsSleeping)
				{
					num++;
				}
