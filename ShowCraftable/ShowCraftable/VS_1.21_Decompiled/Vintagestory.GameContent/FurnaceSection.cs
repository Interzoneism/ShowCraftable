...
public class FurnaceSection : BlockEntityOpenableContainer
{
	...
	private void OnSlowTick(float dt)
	{
		...
		if (Api is ICoreClientAPI)
		{
			...
			float num = 2.5f * (float)burntime * (float)burntime / (float)(getTotalBurnTime() * getTotalBurnTime());
			num += (float)Math.Min(4, airblowtimer / 2);
			while (num > 0.001f && (!(num < 1f) || !(Api.World.Rand.NextDouble() > (double)num)))
			{
				_ = Pos.X;
	...
	public ItemStack getSmeltedOre(ItemSlot oreSlot)
	{
		...
		CombustibleProperties combustibleProps = oreSlot.Itemstack.Collectible.CombustibleProps;
		if (combustibleProps == null)
		{
			return null;
		}
		ItemStack result = combustibleProps.SmeltedStack.ResolvedItemstack.Clone();
		if (combustibleProps.MeltingPoint <= MaxTemperature)
		{
			return result;
		}
		return null;
