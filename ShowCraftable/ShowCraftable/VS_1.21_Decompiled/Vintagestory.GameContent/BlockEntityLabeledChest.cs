...
public class BlockEntityLabeledChest : BlockEntityGenericTypedContainer
{
	...
	public override bool OnPlayerRightClick(IPlayer byPlayer, BlockSelection blockSel)
	{
		if (byPlayer != null && byPlayer.Entity?.Controls?.ShiftKey == true)
		{
			...
			bool flag = activeHotbarSlot != null && activeHotbarSlot.Itemstack?.Class == EnumItemClass.Block && (byPlayer.WorldData.CurrentGameMode == EnumGameMode.Survival || byPlayer.WorldData.CurrentGameMode == EnumGameMode.Creative);
			if (base.Block.PlacedPriorityInteract && flag)
			{
				return false;
			}
		}
	...
	public override void OnReceivedClientPacket(IPlayer player, int packetid, byte[] data)
	{
		if (packetid == 1002)
		{
			...
			text = editSignPacket.Text;
			fontSize = editSignPacket.FontSize;
			color = tempColor;
	...
	public override void OnReceivedServerPacket(int packetid, byte[] data)
	{
		...
		if (packetid == 1000)
		{
			...
			{
				labelrenderer.fontSize = editSignPacket.FontSize;
				labelrenderer.SetNewText(editSignPacket.Text, color);
			}
		}
