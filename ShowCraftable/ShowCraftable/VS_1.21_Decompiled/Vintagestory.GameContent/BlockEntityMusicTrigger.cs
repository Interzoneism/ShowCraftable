...
public class BlockEntityMusicTrigger : BlockEntity
{
	...
	private void onTick1s(float dt)
	{
		...
		{
			int x = (int)capi.World.Player.Entity.Pos.X - Pos.X;
			int y = (int)capi.World.Player.Entity.Pos.Y - Pos.Y;
			int z = (int)capi.World.Player.Entity.Pos.Z - Pos.Z;
			bool flag = false;
			Cuboidi[] array = areas;
			foreach (Cuboidi cuboidi in array)
			{
				flag |= cuboidi.ContainsOrTouches(x, y, z);
			}
			if (flag)
			{
				StartMusic();
	...
	public override void FromTreeAttributes(ITreeAttribute tree, IWorldAccessor worldAccessForResolve)
	{
		...
		if (tree["areas"] is TreeAttribute treeAttribute)
		{
			List<Cuboidi> list = new List<Cuboidi>();
			IAttribute[] values = treeAttribute.Values;
			foreach (IAttribute attribute in values)
			{
				list.Add(new Cuboidi((attribute as IntArrayAttribute).value));
			}
			areas = list.ToArray();
		}
	...
	public override void ToTreeAttributes(ITreeAttribute tree)
	{
		...
		if (areas != null)
		{
			TreeAttribute treeAttribute = (TreeAttribute)(tree["areas"] = new TreeAttribute());
			for (int i = 0; i < areas.Length; i++)
			{
				treeAttribute[i.ToString() ?? ""] = new IntArrayAttribute(areas[i].Coordinates);
			}
		}
	...
	private void StartMusic()
	{
		...
		if (musicTrack == null || !musicTrack.loading)
		{
			...
			if (musicTrack2 == null || musicTrack2.Sound?.IsPlaying != true)
			{
				nowFadingOut = false;
	...
	private void onTrackLoaded(ILoadedSound sound)
	{
		...
		else
		{
			...
			long num = capi.World.ElapsedMilliseconds - startLoadingMs;
			...
			}, (int)Math.Max(0L, 500 - num), permittedWhilePaused: true);
		}
	}
