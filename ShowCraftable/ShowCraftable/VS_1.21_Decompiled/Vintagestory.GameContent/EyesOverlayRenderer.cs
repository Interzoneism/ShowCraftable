...
public class EyesOverlayRenderer : IRenderer, IDisposable
{
	...
	public EyesOverlayRenderer(ICoreClientAPI capi, IShaderProgram eyeShaderProg)
	{
		...
		MeshData customQuadModelData = QuadMeshUtil.GetCustomQuadModelData(-1f, -1f, -19848f, 2f, 2f);
		customQuadModelData.Rgba = null;
		quadRef = capi.Render.UploadMesh(customQuadModelData);
		string text = capi.Input.HotKeys["sneak"].CurrentMapping.ToString();
		exitHelpTexture = capi.Gui.TextTexture.GenTextTexture(Lang.Get("bed-exithint", text), CairoFont.WhiteSmallishText());
	}
