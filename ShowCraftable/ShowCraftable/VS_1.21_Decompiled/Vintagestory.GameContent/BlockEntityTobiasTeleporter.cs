...
public class BlockEntityTobiasTeleporter : BlockEntityTeleporterBase
{
	...
	public override void Initialize(ICoreAPI api)
	{
		...
		if (Api is ICoreServerAPI coreServerAPI)
		{
			...
			if (IsAtTobiasCave)
			{
				...
				Vec3d tobiasTeleporterLocation = Pos.ToVec3d() + BlockTobiasTeleporter.GetTeleportOffset(side);
				SystemTobiasTeleporter.TeleporterData.TobiasTeleporterLocation = tobiasTeleporterLocation;
			}
			else
			{
				...
				sapi.ModLoader.GetModSystem<GenStoryStructures>().storyStructureInstances.TryGetValue("tobiascave", out var value);
				if (value != null && Pos.DistanceTo(value.CenterPos) < 20f)
				{
					IsAtTobiasCave = true;
					OwnerPlayerUid = null;
					OwnerName = null;
				...
				string text = sapi.PlayerData.GetPlayerDataByUid(OwnerPlayerUid)?.LastKnownPlayername;
				if (OwnerName != text && !string.IsNullOrEmpty(text))
				{
					OwnerName = text;
					MarkDirty();
				}
		...
		else
		{
			...
			animUtil.InitializeAnimator("tobiasteleporter", null, null, new Vec3f(0f, rotateY, 0f));
	...
	public override void OnEntityCollide(Entity entity)
	{
		...
		{
			if (entity is EntityPlayer entityPlayer && ((IsAtTobiasCave && SystemTobiasTeleporter.TryGetPlayerLocation(entityPlayer.PlayerUID, out var location)) || (SystemTobiasTeleporter.IsAllowedToTeleport(entityPlayer.PlayerUID, out location) && entityPlayer.PlayerUID.Equals(OwnerPlayerUid))))
			{
				base.OnEntityCollide(entity);
			}
		}
		else if (entity is EntityPlayer entityPlayer2 && somebodyIsTeleporting && ((IsAtTobiasCave && SystemTobiasTeleporter.OwnLastUsage != 0.0) || entityPlayer2.PlayerUID.Equals(OwnerPlayerUid)))
		{
			base.OnEntityCollide(entity);
		}
	}
	...
	public override Vec3d GetTarget(Entity forEntity)
	{
		if (IsAtTobiasCave)
		{
			if (forEntity is EntityPlayer entityPlayer && SystemTobiasTeleporter.TryGetPlayerLocation(entityPlayer.PlayerUID, out var location))
			{
				return location;
			}
		}
		else if (forEntity is EntityPlayer entityPlayer2 && entityPlayer2.PlayerUID.Equals(OwnerPlayerUid))
		{
