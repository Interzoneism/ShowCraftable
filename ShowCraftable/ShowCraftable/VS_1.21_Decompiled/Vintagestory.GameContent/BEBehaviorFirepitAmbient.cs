...
public class BEBehaviorFirepitAmbient : BlockEntityBehavior
{
	...
	private bool HasNearbySittingPlayer
	{
		get
		{
			...
			{
				return player.Entity.Pos.DistanceTo(Blockentity.Pos.ToVec3d().Add(0.5, 0.5, 0.5)) < 4.0;
			}
			return false;
		}
	}
	...
	private void OnTick3s(float dt)
	{
		...
		if (!(totalHours - MusicLastPlayedTotalHr < 6.0) && IsNight && befirepit.IsBurning)
		{
			...
			{
				MusicActive = true;
				MusicLastPlayedTotalHr = totalHours;
