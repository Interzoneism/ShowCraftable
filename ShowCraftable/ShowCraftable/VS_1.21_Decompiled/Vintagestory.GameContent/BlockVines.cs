...
public class BlockVines : Block
{
	...
	public override void OnDecalTesselation(IWorldAccessor world, MeshData decalMesh, BlockPos pos)
	{
		...
		if (blockOnSide.Id != 0 && blockOnSide.CanAttachBlockAt(blockAccessor, this, tmpPos.Set(pos).Add(VineFacing.Opposite), VineFacing) && !(blockOnSide is BlockLeaves))
		{
		...
		int num = ((blockAccessor.GetBlockAbove(pos, 1, 1) is BlockVines) ? 1 : 0) + ((blockAccessor.GetBlockAbove(pos, 2, 1) is BlockVines) ? 1 : 0) + ((blockAccessor.GetBlockAbove(pos, 3, 1) is BlockVines) ? 1 : 0);
		int windDatam = ((num != 3 || !(blockAccessor.GetBlockAbove(pos, 4, 1) is BlockVines)) ? (Math.Max(0, num - 1) << 29) : (num << 29));
		num <<= 29;
		if (blockAccessor.GetBlockAbove(pos, 1, 1) is BlockVines)
		{
	...
	public override void OnJsonTesselation(ref MeshData sourceMesh, ref int[] lightRgbsByCorner, BlockPos pos, Block[] chunkExtBlocks, int extIndex3d)
	{
		...
		int num2 = ((api.World.BlockAccessor.GetBlockAbove(pos, 1, 1) is BlockVines) ? 1 : 0) + ((api.World.BlockAccessor.GetBlockAbove(pos, 2, 1) is BlockVines) ? 1 : 0) + ((api.World.BlockAccessor.GetBlockAbove(pos, 3, 1) is BlockVines) ? 1 : 0);
		int windDatam = ((num2 != 3 || !(api.World.BlockAccessor.GetBlockAbove(pos, 4, 1) is BlockVines)) ? (Math.Max(0, num2 - 1) << 29) : (num2 << 29));
		num2 <<= 29;
	...
	public override bool TryPlaceBlockForWorldGen(IBlockAccessor blockAccessor, BlockPos pos, BlockFacing onBlockFace, IRandom worldGenRand, BlockPatchAttributes attributes = null)
	{
		...
		if (blockAbove is BlockVines)
		{
			BlockFacing vineFacing = ((BlockVines)blockAbove).VineFacing;
