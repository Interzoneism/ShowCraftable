...
public class BEBehaviorJonasGasifier : BlockEntityBehavior, INetworkedLight
{
	...
	private void onTick(float dt)
	{
		...
		double num = Math.Min(2400.0, Api.World.Calendar.TotalHours - burnStartTotalHours);
		while (num > 8.0)
		{
	...
	internal void Interact(IPlayer byPlayer, BlockSelection blockSel)
	{
		...
		if (!activeHotbarSlot.Empty && activeHotbarSlot.Itemstack.Collectible.CombustibleProps != null && activeHotbarSlot.Itemstack.Collectible.CombustibleProps.BurnTemperature >= 1100 && activeHotbarSlot.TryPutInto(Api.World, inventory[0]) > 0)
		{
