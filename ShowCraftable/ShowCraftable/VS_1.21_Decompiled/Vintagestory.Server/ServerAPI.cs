...
internal class ServerAPI : ServerAPIComponentBase, IServerAPI
{
	...
	public bool ReducedServerThreads => server.ReducedServerThreads;
	...
	{
		ServerThread serverThread = server.ServerThreadLoops.FirstOrDefault((ServerThread val) => val.threadName == threadname);
		serverThread.ShouldPause = true;
		while (waitTimeoutMs > 0 && !serverThread.paused)
		{
			Thread.Sleep(50);
			waitTimeoutMs -= 50;
		}
		return serverThread.paused;
	}
	...
	public int LoadMiniDimension(IMiniDimension blocks)
	{
		...
		int num = ++server.SaveGameData.MiniDimensionsCreated;
		server.SetMiniDimension(blocks, num);
		return num;
	}
