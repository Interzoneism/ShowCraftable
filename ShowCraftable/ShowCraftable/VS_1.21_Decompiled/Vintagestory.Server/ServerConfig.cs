...
public class ServerConfig : IServerConfig
{
	...
	public int MaxClients
	{
		get
		{
			if (MaxClientsProgArgs == -1)
			{
				return maxClients;
			}
			return MaxClientsProgArgs;
		}
		set
		{
			maxClients = value;
	...
	internal void OnDeserializedMethod(StreamingContext context)
	{
		...
		if (GameVersion.IsLowerVersionThan(ConfigVersion, "1.3"))
		{
			...
			{
				if (!item.Value.AutoGrant)
				{
					item.Value.Privileges.Add(Privilege.attackcreatures);
					item.Value.Privileges.Add(Privilege.attackplayers);
				}
			}
		}
		...
		if (GameVersion.IsLowerVersionThan(ConfigVersion, "1.8") && RuntimeEnv.OS == OS.Mac)
		{
			...
			if (WorldConfig?.SaveFileLocation != null)
			{
				WorldConfig.SaveFileLocation = WorldConfig.SaveFileLocation.Replace(text, GamePaths.Config);
			}
		}
	...
	public void ApplyStartServerArgs(StartServerArgs serverargs)
	{
		...
		{
			if (int.TryParse(WorldConfig.Seed, out var result))
			{
				Seed = result;
			}
			else
