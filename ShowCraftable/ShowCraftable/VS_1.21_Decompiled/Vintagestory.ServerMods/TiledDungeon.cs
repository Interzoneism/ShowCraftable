...
public class TiledDungeon
{
	...
	internal void Init(ICoreServerAPI api)
	{
		...
		instance.ResolveBlockIds(api);
		if (stairs != null)
		{
			IAsset asset = api.Assets.Get("worldgen/dungeontiles/" + stairs + ".json");
			Stairs = WorldGenStructureBase.LoadSchematic<BlockSchematicPartial>(api, asset, instance, null, null, 0);
		}
		if (start != null)
		{
			IAsset asset2 = api.Assets.Get("worldgen/dungeontiles/" + start + ".json");
			Start = WorldGenStructureBase.LoadSchematic<BlockSchematicPartial>(api, asset2, instance, null, null, 0, isDungeon: true);
