...
public class MainMenuGuiAPI : IGuiAPI
{
	...
	{
		MainMenuGuiAPI mainMenuGuiAPI = this;
		this.screenManager = screenManager;
		prober = new TextDrawUtil();
		textutil = new TextTextureUtil(capi);
		iconutil = new IconUtil(capi);
		svgLoader = new SvgLoader(capi);
		Icons.CustomIcons["copy"] = delegate(Context ctx, int x, int y, float w, float h, double[] rgba)
		{
			MainMenuGuiAPI mainMenuGuiAPI2 = mainMenuGuiAPI;
			IAsset svgAsset = screenManager.GamePlatform.AssetManager.TryGet_BaseAssets("textures/icons/copy.svg");
			Surface target = ctx.GetTarget();
			mainMenuGuiAPI2.DrawSvg(svgAsset, (ImageSurface)(object)((target is ImageSurface) ? target : null), ctx.Matrix, x, y, (int)w, (int)h, ColorUtil.FromRGBADoubles(rgba));
		};
		Icons.CustomIcons["paste"] = delegate(Context ctx, int x, int y, float w, float h, double[] rgba)
		{
			MainMenuGuiAPI mainMenuGuiAPI2 = mainMenuGuiAPI;
			IAsset svgAsset = screenManager.GamePlatform.AssetManager.TryGet_BaseAssets("textures/icons/paste.svg");
			Surface target = ctx.GetTarget();
			mainMenuGuiAPI2.DrawSvg(svgAsset, (ImageSurface)(object)((target is ImageSurface) ? target : null), ctx.Matrix, x, y, (int)w, (int)h, ColorUtil.FromRGBADoubles(rgba));
		};
	}
